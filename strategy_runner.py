...
try:
    ...
    # getpass mungkin tidak selalu ada, handle jika tidak ada
    pass
...
def animated_text_display(text, delay=0.02, color=AnsiColors.CYAN, new_line=True, bold=False, center=False, width=None):
    ...
    if center:
        # Untuk efek ketik dengan centering, cetak padding dulu, lalu karakter
        padding = (width - len(text)) // 2
        padding = max(0, padding) # Pastikan padding tidak negatif
    ...
    for char_index, char in enumerate(text):
        ...
        output_char += char
        output_char += AnsiColors.ENDC # Selalu reset setelah karakter berwarna/bold
...
def show_spinner(duration_seconds, message="Processing...", color=AnsiColors.MAGENTA):
    ...
    while (time.time() - start_time) < duration_seconds:
        ...
        # Gabungkan warna dan bold sebelum loop untuk efisiensi
    ...
    sys.stdout.write(AnsiColors.ENDC) # Pastikan reset di akhir
    sys.stdout.flush()
...
def simple_progress_bar(iteration, total, prefix='', suffix='', decimals=1, length=30, fill='❚', print_end="\r"):
    ...
    try:
        ...
        # Format: Prefix |Bar| Percent% Suffix
        # Available space for bar = term_width - len_no_ansi(prefix) - len_no_ansi(suffix) - 2 (for |) - 7 (for " 100.0% ") - 2 (buffer)
        # Ini bisa rumit karena prefix/suffix bisa berwarna. Simpelkan:
        non_bar_len = len(prefix.replace('\033[', '').split('m')[-1]) + \
                      len(suffix.replace('\033[', '').split('m')[-1]) + \
                      12 # Ruang untuk persentase, spasi, dan buffer
        length = max(10, min(length, term_width - non_bar_len))
    except OSError:
        pass 
    ...
    bar_chars = fill * filled_length + '░' * (length - filled_length) 

    bar_color = AnsiColors.GREEN
    if percent_val < 30: bar_color = AnsiColors.ORANGE
    elif percent_val < 70: bar_color = AnsiColors.YELLOW_BG + AnsiColors.BLUE # Contoh kombinasi
    ...
    # Pad to clear previous line, then \r
    sys.stdout.write(f"\r{progress_line:{term_width if 'term_width' in locals() else 80}}")
    sys.stdout.flush()
...
console_formatter_template = (
    ...
    f"%(log_color)s%(message_content)s{AnsiColors.ENDC}" # Ganti %(message)s ke %(message_content)s
)
...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Simpan pesan asli sebelum dimodifikasi untuk formatter lain (misal file handler)
        original_message = record.getMessage()
        
        # Buat atribut baru untuk pesan yang akan diwarnai di console
        record.message_content = original_message
        
        # Hanya tambahkan emoji jika pesan belum diawali dengan kode ANSI
        if not original_message.startswith('\033['):
             record.message_content = level_emoji_map.get(record.levelno, "") + original_message
        
        # Kembalikan ke formatter default untuk %(message)s agar tidak mempengaruhi file log
        # record.message = original_message -> ini tidak perlu karena formatter akan panggil getMessage() lagi
...
class APIKeyManager:
    def __init__(self, primary_key, recovery_keys_list, global_settings_for_email=None):
        ...
        if recovery_keys_list: # Filter hanya string non-kosong
...
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            else: settings["api_settings"] = {**defaults, **settings["api_settings"]} # Merge, prioritaskan yang ada di file
            if "cryptos" not in settings or not isinstance(settings["cryptos"], list): settings["cryptos"] = []
            for cfg in settings["cryptos"]: # Pastikan ID dan enabled ada
            ...
            return {"api_settings": defaults.copy(), "cryptos": [get_default_crypto_config()]} # Beri contoh 1 crypto
    else: # File tidak ada
        log_info(f"File {SETTINGS_FILE} tidak ditemukan. Membuat dengan default.")
        new_settings = {"api_settings": defaults.copy(), "cryptos": []}
        save_settings(new_settings) # Simpan file default baru
        return new_settings
...
def termux_input(prompt_text, current_value="", color=AnsiColors.CYAN, highlight_color=AnsiColors.WHITE, is_password=False, prefix_icon="▸"):
    """Input kustom untuk Termux dengan pewarnaan dan default value."""
    ...
    user_input = ""
    if is_password:
        try: # Coba getpass jika tersedia
            if 'getpass' in sys.modules:
                user_input = getpass.getpass(prompt=full_prompt)
            else: raise ImportError # Paksa ke fallback jika getpass tidak diimport
        except (ImportError, RuntimeError, EOFError): # Runtime can happen in some environments
    ...
    sys.stdout.write(AnsiColors.ENDC) # Reset warna setelah input
    return user_input or str(current_value) # Kembalikan str(current_value) agar konsisten

def _prompt_crypto_config(current_config):
    clear_screen_animated()
    new_cfg = current_config.copy() # Bekerja pada salinan
    sym = new_cfg.get('symbol','BARU')
    cur = new_cfg.get('currency','BARU')
    ...
    print(AnsiColors.GREY + ("-" * (os.get_terminal_size().columns if os.isatty() else 60)) + AnsiColors.ENDC)
    ...
    except ValueError: log_warning(f"Input refresh interval tidak valid, menggunakan default.") # Default sudah di new_cfg
    print()
...
def settings_menu(current_settings):
    while True:
        ...
        pk_val = api_s.get('primary_key', '')
        pk_disp = "BELUM DIATUR"
        if pk_val and pk_val not in ["YOUR_PRIMARY_KEY", "YOUR_API_KEY_HERE"]:
            pk_disp = pk_val[:5] + "..." + pk_val[-3:] if len(pk_val) > 8 else pk_val
        
        rec_keys = [k for k in api_s.get('recovery_keys', []) if isinstance(k, str) and k.strip()]
        num_rec_keys = len(rec_keys)

        term_width = os.get_terminal_size().columns if os.isatty() else 60
        ...
        g_email_active = api_s.get('enable_global_email_notifications_for_key_switch', False) and \
                         api_s.get('email_sender_address') and api_s.get('email_receiver_address_admin')
        g_email_stat = "Aktif" if g_email_active else "Nonaktif"
        g_email_color = AnsiColors.GREEN if g_email_active else AnsiColors.ORANGE
        ...
        if not current_settings["cryptos"]:
            title_pick += AnsiColors.ORANGE + "  (Belum ada konfigurasi crypto)\n" + AnsiColors.ENDC
        ...
        try:
            opt_txt, idx = pick(options, title_pick, indicator=f'{AnsiColors.GREEN}▶{AnsiColors.ENDC}', default_index=0)
        except (KeyboardInterrupt, Exception) as e_pick: # Tangkap semua exception dari pick
            ...
            show_spinner(1.5, "Error pick, kembali...", color=AnsiColors.RED)
            return current_settings # Kembali jika error

        clear_screen_animated()
        try:
            if idx == 0: # Atur Primary API Key
                ...
                api_s["primary_key"] = new_pk if new_pk else "YOUR_PRIMARY_KEY" # Jangan biarkan kosong
                save_settings(current_settings)
            elif idx == 1: # Kelola Recovery API Keys
                while True: # Loop untuk sub-menu recovery keys
                    clear_screen_animated()
                    api_s_rec = current_settings.get("api_settings", {}) # Muat ulang
                    ...
                    try:
                        rec_opt_text, rec_idx = pick(rec_options, rec_pick_title, indicator=f'{AnsiColors.GREEN}▶{AnsiColors.ENDC}')
                    except (KeyboardInterrupt, Exception) as e_rec_pick:
                        if isinstance(e_rec_pick, KeyboardInterrupt): break # Keluar dari sub-menu
                        log_warning(f"Error pick di menu recovery key: {e_rec_pick}")
                        show_spinner(1, "Error, kembali...")
                    ...
                    if rec_idx == 0: # Tambah
                        ...
                        if new_rk_val and new_rk_val not in current_rec_keys:
                            current_rec_keys.append(new_rk_val)
                            api_s_rec['recovery_keys'] = current_rec_keys # Update dict lokal dulu
                            current_settings['api_settings']['recovery_keys'] = current_rec_keys # Update dict utama
                            save_settings(current_settings)
                        ...
                        if not current_rec_keys: log_warning("Tidak ada recovery key untuk dihapus.")
                            ...
                            try:
                                ...
                                if del_idx_rk < len(current_rec_keys): # Bukan "Batal"
                                    removed_rk = current_rec_keys.pop(del_idx_rk)
                                    api_s_rec['recovery_keys'] = current_rec_keys
                                    current_settings['api_settings']['recovery_keys'] = current_rec_keys
                                    save_settings(current_settings)
                                    log_info(f"Recovery key '{removed_rk[:5]}...' dihapus.")
                                else: log_info("Penghapusan dibatalkan.")
                            except (KeyboardInterrupt, Exception) as e_del_rk_pick: # Tangkap jika pick gagal/dibatalkan
                    ...
                    elif rec_idx == 2: break # Kembali ke menu pengaturan utama
                    if rec_idx in [0,1]: show_spinner(1.5, "Memproses...", color=AnsiColors.BLUE)

            elif idx == 2: # Atur Email Global
                animated_text_display("-- 📧 Pengaturan Email Global --", color=AnsiColors.HEADER, bold=True, center=True)
                print(AnsiColors.GREY + "Email untuk notifikasi sistem (ganti API key, dll)." + AnsiColors.ENDC)
                print(AnsiColors.GREY + ("-" * term_width) + AnsiColors.ENDC)
                ...
                api_s['email_receiver_address_admin'] = termux_input("Email Penerima Notifikasi Sistem (Admin)", api_s.get('email_receiver_address_admin',''))
                save_settings(current_settings)
            elif idx == 3: # Tambah Crypto
                new_crypto = _prompt_crypto_config(get_default_crypto_config()) 
                current_settings["cryptos"].append(new_crypto)
                save_settings(current_settings)
                log_info(f"Konfigurasi {new_crypto['symbol']}-{new_crypto['currency']} ditambahkan.")
            elif idx == 4: # Ubah Crypto
                if not current_settings["cryptos"]: log_warning("Tidak ada konfigurasi untuk diubah.")
                    ...
                    try:
                        ...
                        if idx_edit < len(current_settings["cryptos"]):
                            current_settings["cryptos"][idx_edit] = _prompt_crypto_config(current_settings["cryptos"][idx_edit])
                            save_settings(current_settings)
                    ...
                    except (KeyboardInterrupt, Exception) as e_edit_c_pick: # Tangkap jika pick gagal/dibatalkan
                ...
                if not current_settings["cryptos"]: log_warning("Tidak ada konfigurasi untuk dihapus.")
                    ...
                    try:
                        ...
                        if idx_del < len(current_settings["cryptos"]):
                            removed = current_settings["cryptos"].pop(idx_del)
                            save_settings(current_settings)
                            log_info(f"Konfigurasi {removed['symbol']}-{removed['currency']} dihapus.")
                        else: log_info("Penghapusan dibatalkan.")
                    except (KeyboardInterrupt, Exception) as e_del_c_pick: # Tangkap jika pick gagal/dibatalkan
            ...
            elif idx == 6: break # Kembali ke Menu Utama
            
            if idx < 6 : show_spinner(1.5, "Menyimpan & Kembali...", color=AnsiColors.GREEN if idx not in [0,1,2] else AnsiColors.BLUE) # idx 0,1,2 sudah ada spinner sendiri
        
        except ValueError:
            log_error("Input angka tidak valid.")
            show_spinner(1.5, "Error, kembali...", color=AnsiColors.RED)
        except Exception as e_menu_action:
            log_error(f"Error di menu pengaturan: {e_menu_action}")
            log_exception("Traceback Pengaturan:")
...
def fetch_candles(symbol, currency, total_limit_desired, exchange_name, current_api_key_to_use, timeframe="hour", pair_name="N/A"):
    ...
    is_large = total_limit_desired > 20 # Threshold untuk progress bar
    ...
    while len(all_candles) < total_limit_desired:
        ...
        try:
            if is_large and total_limit_desired > CRYPTOCOMPARE_MAX_LIMIT: 
                 log_debug(f"Batch (Key: ...{key_short}, Limit: {limit})", pair_name=pair_name)
            ...
            if resp.status_code in [401, 403, 429]: 
                ...
                log_warning(f"API Key Error (HTTP {resp.status_code}): {err_msg} Key: ...{key_short}", pair_name=pair_name)
                raise APIKeyError(f"HTTP {resp.status_code}: {err_msg}")
            ...
            if data.get('Response') == 'Error':
                ...
                key_errs = ["api key is invalid", "apikey_is_missing", "apikey_invalid", "monthly_calls", "rate limit", "pro_tier"]
                if any(k_err.lower() in err_msg.lower() for k_err in key_errs):
                    log_warning(f"API Key Error (JSON): {err_msg} Key: ...{key_short}", pair_name=pair_name)
                    raise APIKeyError(f"JSON Error: {err_msg}")
                else:
                    log_error(f"API Error CryptoCompare: {err_msg} (Params: {params})", pair_name=pair_name)
                    break 
            
            if not data.get('Data') or not data['Data'].get('Data'): # Pengecekan data lebih aman
                log_info(f"Tidak ada data candle dari API atau format salah. Diambil: {len(all_candles)}.", pair_name=pair_name)
                break 
            
            raw_api_candles = data['Data']['Data']
            if not raw_api_candles: # Jika list kosong
                log_info(f"API mengembalikan list candle kosong. Diambil: {len(all_candles)}.", pair_name=pair_name)
                break

            batch = []
            for item in raw_api_candles:
                if not all(k in item for k in ['time', 'open', 'high', 'low', 'close']):
                    log_warning(f"Data candle tidak lengkap dari API: {item}. Dilewati.", pair_name=pair_name)
                    continue
                batch.append({'timestamp': datetime.fromtimestamp(item['time']), 'open': item['open'], 'high': item['high'],
                              'low': item['low'], 'close': item['close'], 'volume': item.get('volumefrom')})

            if to_ts and all_candles and batch and batch[-1]['timestamp'] == all_candles[0]['timestamp']:
                if is_large: log_debug(f"Hapus overlap: {batch[-1]['timestamp']}", pair_name=pair_name)
                batch.pop() 
            
            if not batch and to_ts : 
                if is_large: log_info("Batch kosong setelah overlap removal. Akhir data.", pair_name=pair_name)
                break

            all_candles = batch + all_candles
            if raw_api_candles: to_ts = raw_api_candles[0]['time'] 
            else: break # Seharusnya tidak terjadi
            ...
            if len(raw_api_candles) < limit: # API mengembalikan lebih sedikit dari yang diminta
                if is_large: log_info(f"API mengembalikan < limit ({len(raw_api_candles)} vs {limit}). Akhir histori.", pair_name=pair_name)
                break 
            if len(all_candles) >= total_limit_desired: break 
            if len(all_candles) < total_limit_desired and total_limit_desired > CRYPTOCOMPARE_MAX_LIMIT and is_large:
                time.sleep(0.25) # Jeda antar batch besar
        except APIKeyError: raise 
        except requests.exceptions.RequestException as e:
            log_error(f"Kesalahan koneksi saat ambil batch: {e}", pair_name=pair_name)
            break 
        except Exception as e:
            log_error(f"Error tak terduga fetch_candles: {e}", pair_name=pair_name)
            log_exception("Traceback Fetch Candles:", pair_name=pair_name) 
            break 

    if len(all_candles) > total_limit_desired:
        all_candles = all_candles[-total_limit_desired:]

    if is_large and total_limit_desired > CRYPTOCOMPARE_MAX_LIMIT:
        simple_progress_bar(len(all_candles), total_limit_desired, prefix=f'{AnsiColors.BOLD}{pair_name}{AnsiColors.ENDC} Data:', suffix='Selesai', length=20)
    if is_large or total_limit_desired <=5 : # Log jika fetch kecil juga
      log_info(f"Pengambilan data selesai. Total {AnsiColors.GREEN}{len(all_candles)}{AnsiColors.ENDC} (target: {total_limit_desired}).", pair_name=pair_name)
...
def find_pivots(series_list, left_strength, right_strength, is_high=True):
    ...
    for i in range(left_strength, len(series_list) - right_strength):
        val_i = series_list[i]
        if val_i is None: continue # Lewati jika data titik tengah None
        is_pivot = True
        # Kiri
        for j in range(1, left_strength + 1):
            val_ij = series_list[i-j]
            if val_ij is None : is_pivot = False; break # Lewati jika data pembanding None
            if (is_high and val_i <= val_ij) or (not is_high and val_i >= val_ij):
        ...
        for j in range(1, right_strength + 1):
            ...
            if (is_high and val_i < val_ij) or (not is_high and val_i > val_ij): # Ketat untuk kanan
                is_pivot = False; break
        if is_pivot: pivots[i] = val_i
    return pivots


def run_strategy_logic(candles, cfg, state): # Ubah nama parameter agar lebih pendek
    pair = f"{cfg['symbol']}-{cfg['currency']}"
    state["final_pivot_high_price_confirmed"] = None
    state["final_pivot_low_price_confirmed"] = None

    req_keys = ['high', 'low', 'open', 'close', 'timestamp']
    if not candles or not (candles[0] and all(k in candles[0] for k in req_keys)):
        log_warning(f"Data candle kosong/tidak lengkap.", pair_name=pair); return state

    # Ekstrak harga dengan penanganan None
    highs = [c.get('high') for c in candles]
    lows = [c.get('low') for c in candles]

    # Find Pivots akan menangani None dalam series
    piv_highs = find_pivots(highs, cfg['left_strength'], cfg['right_strength'], True)
    piv_lows = find_pivots(lows,  cfg['left_strength'], cfg['right_strength'], False)

    curr_idx = len(candles) - 1 
    if curr_idx < 0: return state

    idx_piv_event_h = curr_idx - cfg['right_strength']
    idx_piv_event_l = curr_idx - cfg['right_strength']

    # Dapatkan harga pivot mentah pada index konfirmasi, aman dari IndexError
    ...
    if raw_piv_h_price is not None and state["last_signal_type"] != 1:
        state["final_pivot_high_price_confirmed"] = raw_piv_h_price
        state["last_signal_type"] = 1 
        if 0 <= idx_piv_event_h < len(candles) and candles[idx_piv_event_h] and candles[idx_piv_event_h].get('timestamp'):
            ts = candles[idx_piv_event_h]['timestamp']
            log_info(f"{AnsiColors.CYAN}📈 PIVOT HIGH: {state['final_pivot_high_price_confirmed']:.5f} @ {ts.strftime('%H:%M')}{AnsiColors.ENDC}", pair_name=pair)
    ...
    if raw_piv_l_price is not None and state["last_signal_type"] != -1:
        state["final_pivot_low_price_confirmed"] = raw_piv_l_price
        state["last_signal_type"] = -1 
        if 0 <= idx_piv_event_l < len(candles) and candles[idx_piv_event_l] and candles[idx_piv_event_l].get('timestamp'):
            ts = candles[idx_piv_event_l]['timestamp']
            log_info(f"{AnsiColors.CYAN}📉 PIVOT LOW:  {state['final_pivot_low_price_confirmed']:.5f} @ {ts.strftime('%H:%M')}{AnsiColors.ENDC}", pair_name=pair)
    ...
    if not curr_c or any(curr_c.get(k) is None for k in ['open', 'high', 'low', 'close']):
        ts_str = curr_c.get('timestamp', datetime.now()).strftime('%H:%M')
        log_warning(f"Data OHLC tidak lengkap untuk candle terbaru @ {ts_str}. Skip.", pair_name=pair); return state

    # Fibonacci & Entry
    if state["final_pivot_high_price_confirmed"] is not None:
        state["high_price_for_fib"] = state["final_pivot_high_price_confirmed"]
        state["high_bar_index_for_fib"] = idx_piv_event_h
        if state["active_fib_level"] is not None: # Reset FIB aktif jika ada PH baru
            log_debug("Reset active FIB (new High).", pair_name=pair)
            state.update({"active_fib_level": None, "active_fib_line_start_index": None})

    if state["final_pivot_low_price_confirmed"] is not None and \
       state["high_price_for_fib"] is not None and \
       state["high_bar_index_for_fib"] is not None and \
       idx_piv_event_l > state["high_bar_index_for_fib"]: # PL setelah PH
        
        h_fib, l_fib = state["high_price_for_fib"], state["final_pivot_low_price_confirmed"]
        calc_fib = (h_fib + l_fib) / 2.0
        
        is_late = False
        if cfg["enable_secure_fib"]:
            price_check = curr_c.get(cfg["secure_fib_check_price"].lower(), curr_c['close'])
            if price_check is not None and price_check > calc_fib: is_late = True
        
        if is_late:
            price_disp = f"{price_check:.5f}" if price_check is not None else "N/A"
            log_info(f"{AnsiColors.ORANGE}⏳ FIB Terlambat ({calc_fib:.5f}), Cek({cfg['secure_fib_check_price']}:{price_disp}) > FIB.{AnsiColors.ENDC}", pair_name=pair)
            state.update({"active_fib_level": None, "active_fib_line_start_index": None})
        else:
            log_info(f"{AnsiColors.CYAN}✨ FIB 0.5 Aktif: {calc_fib:.5f}{AnsiColors.ENDC} (H:{h_fib:.2f},L:{l_fib:.2f})", pair_name=pair)
            state.update({"active_fib_level": calc_fib, "active_fib_line_start_index": idx_piv_event_l})
        
        state.update({"high_price_for_fib": None, "high_bar_index_for_fib": None}) # Reset PH untuk FIB

    # Cek Entry
    if state["active_fib_level"] is not None and state["position_size"] == 0 and \
       curr_c['close'] > curr_c['open'] and curr_c['close'] > state["active_fib_level"]:
        
        entry_px = curr_c['close']
        emerg_sl = entry_px * (1 - cfg["emergency_sl_percent"] / 100.0)
        state.update({
            "position_size": 1, "entry_price_custom": entry_px,
            "highest_price_for_trailing": entry_px, "trailing_tp_active_custom": False,
            "current_trailing_stop_level": None, "emergency_sl_level_custom": emerg_sl,
            "active_fib_level": None, "active_fib_line_start_index": None # Nonaktifkan FIB setelah entry
        })
        
        log_msg = f"🚀 BUY ENTRY @ {entry_px:.5f} (FIB {state.get('active_fib_level', calc_fib):.5f} dilewati). SL: {emerg_sl:.5f}" # active_fib_level sdh None
        log_info(f"{AnsiColors.GREEN}{AnsiColors.BOLD}{log_msg}{AnsiColors.ENDC}", pair_name=pair)
        play_notification_sound(count=2, freq=1200, dur=150)
        
        ts_email = curr_c['timestamp'].strftime('%Y-%m-%d %H:%M:%S')
        send_email_notification(f"BUY Signal: {pair}", 
                                f"New BUY: {pair} @ {entry_px:.5f}\nFIB Lvl: {state.get('active_fib_level', calc_fib):.5f}\nEmerg SL: {emerg_sl:.5f}\nTime: {ts_email}",
                                {**cfg, 'pair_name': pair}) # Pastikan pair_name ada untuk email context

    # Manajemen Posisi
    if state["position_size"] > 0:
        # Pastikan nilai tidak None sebelum operasi
        highest_trailing = state.get("highest_price_for_trailing", curr_c.get('high', 0))
        current_high = curr_c.get('high', highest_trailing) # Default ke yg sudah ada jika high None
        state["highest_price_for_trailing"] = max(highest_trailing, current_high)

        entry_p = state.get("entry_price_custom")
        if not state["trailing_tp_active_custom"] and entry_p is not None and entry_p != 0:
            ...
            if profit_pct >= cfg["profit_target_percent_activation"]:
                state["trailing_tp_active_custom"] = True
                log_info(f"{AnsiColors.BLUE}🛡️ Trailing TP Aktif. Profit: {profit_pct:.2f}%, High: {state['highest_price_for_trailing']:.5f}{AnsiColors.ENDC}", pair_name=pair)
        ...
        if state["trailing_tp_active_custom"] and state["highest_price_for_trailing"] is not None:
            ...
            if state.get("current_trailing_stop_level") is None or new_stop > state["current_trailing_stop_level"]:
                state["current_trailing_stop_level"] = new_stop
                log_debug(f"Trailing SL update: {new_stop:.5f}", pair_name=pair)

        stop_exit = state.get("emergency_sl_level_custom")
        exit_reason, exit_clr = "Emergency SL", AnsiColors.RED
        if state["trailing_tp_active_custom"] and state.get("current_trailing_stop_level") is not None:
            if stop_exit is None or state["current_trailing_stop_level"] > stop_exit:
                stop_exit = state["current_trailing_stop_level"]
                exit_reason, exit_clr = "Trailing Stop", AnsiColors.BLUE
        
        curr_low = curr_c.get('low')
        if stop_exit is not None and curr_low is not None and curr_low <= stop_exit:
            exit_px = min(curr_c.get('open', stop_exit), stop_exit)
            pnl = 0.0
            if entry_p is not None and entry_p != 0:
                pnl = ((exit_px - entry_p) / entry_p) * 100.0
            
            if exit_reason == "Trailing Stop" and pnl < 0: exit_clr = AnsiColors.RED
            pnl_clr = AnsiColors.GREEN if pnl >=0 else AnsiColors.RED
            log_msg = f"🛑 EXIT @ {exit_px:.5f} by {exit_reason}. PnL: {pnl_clr}{pnl:.2f}%{AnsiColors.ENDC}"
            log_info(f"{exit_clr}{AnsiColors.BOLD}{log_msg}", pair_name=pair)
            play_notification_sound(freq=800, dur=300)

            ts_email_exit = curr_c['timestamp'].strftime('%Y-%m-%d %H:%M:%S')
            send_email_notification(f"Trade Closed: {pair} ({exit_reason})",
                                    f"Trade Closed: {pair}\nExit: {exit_px:.5f} by {exit_reason}\nEntry: {entry_p if entry_p else 0:.5f}\nPnL: {pnl:.2f}%\nTime: {ts_email_exit}",
                                    {**cfg, 'pair_name': pair})
            
            # Reset state posisi
            state.update(get_initial_strategy_state()) # Reset ke state awal untuk posisi
            state["last_signal_type"] = 0 # Reset juga last_signal_type setelah trade selesai

    # Log status posisi aktif
    if state["position_size"] > 0:
        sl_plot = state.get("emergency_sl_level_custom")
        sl_type = "Emergency SL"
        if state.get("trailing_tp_active_custom") and state.get("current_trailing_stop_level") is not None:
            if sl_plot is None or state.get("current_trailing_stop_level") > sl_plot:
                sl_plot = state.get("current_trailing_stop_level")
                sl_type = "Trailing SL"
        
        entry_disp = state.get('entry_price_custom', 0)
        sl_disp = f'{sl_plot:.5f} ({sl_type})' if sl_plot is not None else 'N/A'
        
        curr_close_pnl = curr_c.get('close', 0)
        curr_pnl_val = 0.0
        entry_p_log = state.get("entry_price_custom")
        if entry_p_log and curr_close_pnl != 0 and entry_p_log !=0: # Pastikan entry_p_log tidak 0
            curr_pnl_val = ((curr_close_pnl - entry_p_log) / entry_p_log) * 100.0
        
        pnl_color_log = AnsiColors.GREEN if curr_pnl_val >=0 else AnsiColors.ORANGE
        log_debug(f"Posisi Aktif. Entry: {entry_disp:.5f}, SL: {sl_disp}, PnL: {pnl_color_log}{curr_pnl_val:.2f}%{AnsiColors.ENDC}", pair_name=pair)
    return state
...
def start_trading(global_settings):
    clear_screen_animated() 
    api_cfg = global_settings.get("api_settings", {})
    key_manager = APIKeyManager(api_cfg.get("primary_key"), api_cfg.get("recovery_keys", []), api_cfg)

    if not key_manager.has_valid_keys():
        log_error(f"Tidak ada API key valid. Tidak dapat memulai.")
        animated_text_display("Tekan Enter...", color=AnsiColors.ORANGE, center=True, bold=True); input()
        return

    active_cfgs = [c for c in global_settings.get("cryptos", []) if c.get("enabled", True)]
    if not active_cfgs:
        log_warning(f"Tidak ada konfigurasi crypto aktif."); input()
        animated_text_display("Tekan Enter...", color=AnsiColors.ORANGE, center=True, bold=True); input()
        return
    
    term_w = os.get_terminal_size().columns if os.isatty() else 60
    header_footer = AnsiColors.HEADER + AnsiColors.BOLD + "═" * term_w + AnsiColors.ENDC
    print(header_footer)
    animated_text_display("🚀 MULTI-CRYPTO STRATEGY START 🚀", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002)
    print(header_footer)
    
    key_val = key_manager.get_current_key()
    key_disp = "N/A"
    ...
    for cfg_item in active_cfgs:
        p_id = f"{cfg_item['symbol']}-{cfg_item['currency']}_{cfg_item['timeframe']}"
        cfg_item['pair_name'] = f"{cfg_item['symbol']}-{cfg_item['currency']}" # Pastikan ada
        ...
        data_mgr[p_id] = {"config": cfg_item, "all_candles_list": [], "strategy_state": get_initial_strategy_state(),
        ...
        # Initial Data Fetch
        target_init = TARGET_BIG_DATA_CANDLES 
        initial_data, retries_init, fetched_ok = [], 0, False
        max_retries_init_val = key_manager.total_keys() if key_manager.total_keys() > 0 else 1
        ...
        while retries_init < max_retries_init_val and not fetched_ok:
            ...
            try:
                ...
                initial_data = fetch_candles(cfg_item['symbol'], cfg_item['currency'], target_init, cfg_item['exchange'], curr_key, cfg_item['timeframe'], pair_name=cfg_item['pair_name'])
                fetched_ok = True
            except APIKeyError:
                log_warning(f"BIG DATA: API Key gagal untuk {cfg_item['pair_name']}. Coba berikutnya.", pair_name=cfg_item['pair_name'])
                if not key_manager.switch_to_next_key(): break 
                retries_init +=1 
            except requests.exceptions.RequestException as e_req:
                log_error(f"BIG DATA: Error jaringan {cfg_item['pair_name']}: {e_req}. Tidak ganti key.", pair_name=cfg_item['pair_name']); break 

        if not initial_data:
            log_error(f"BIG DATA: Gagal ambil data awal {cfg_item['pair_name']}. Pair mungkin tidak diproses.", pair_name=cfg_item['pair_name'])
            data_mgr[p_id].update({"big_data_collection_phase_active": False, "last_candle_fetch_time": datetime.now()})
            continue 
        
        data_mgr[p_id]["all_candles_list"] = initial_data
        log_info(f"BIG DATA: {AnsiColors.GREEN}{len(initial_data)}{AnsiColors.ENDC} candle awal diterima.", pair_name=cfg_item['pair_name'])

        # Warm-up Strategy State
        if initial_data:
            min_len_piv = cfg_item['left_strength'] + cfg_item['right_strength'] + 1
            if len(initial_data) >= min_len_piv:
                log_info(f"Warm-up state dengan {max(0, len(initial_data) - 1)} candle historis...", pair_name=cfg_item['pair_name'])
                total_warmup = max(0, len(initial_data) - 1 - (min_len_piv -1))
                if total_warmup > 5: # Hanya tampilkan progress jika > 5 langkah
                    simple_progress_bar(0, total_warmup, prefix=f"{cfg_item['pair_name']} Warm-up:", suffix="State", length=15)

                for i in range(min_len_piv -1, len(initial_data) - 1): 
                    hist_slice = initial_data[:i+1]
                    if len(hist_slice) < min_len_piv: continue 
                    temp_state = data_mgr[p_id]["strategy_state"].copy()
                    temp_state["position_size"] = 0 # Pastikan tidak ada posisi aktif saat warm-up
                    data_mgr[p_id]["strategy_state"] = run_strategy_logic(hist_slice, cfg_item, temp_state)
                    
                    # Reset posisi jika terpicu selama warm-up
                    if data_mgr[p_id]["strategy_state"]["position_size"] > 0: 
                        data_mgr[p_id]["strategy_state"].update(get_initial_strategy_state())
                        data_mgr[p_id]["strategy_state"]["last_signal_type"] = 0 # Reset last signal juga
            ...
            else: log_warning(f"Data awal ({len(initial_data)}) tidak cukup untuk warm-up pivot (min: {min_len_piv}).", pair_name=cfg_item['pair_name'])
        
        if len(data_mgr[p_id]["all_candles_list"]) >= TARGET_BIG_DATA_CANDLES:
            data_mgr[p_id]["big_data_collection_phase_active"] = False
            log_info(f"{AnsiColors.GREEN}🎯 TARGET {TARGET_BIG_DATA_CANDLES} CANDLE TERCAPAI!{AnsiColors.ENDC}", pair_name=cfg_item['pair_name'])
            if not data_mgr[p_id]["big_data_email_sent"]:
                 send_email_notification(f"Data Complete: {cfg_item['pair_name']}", f"Download {TARGET_BIG_DATA_CANDLES} candles selesai! Trading aktif.", {**cfg_item, 'pair_name': cfg_item['pair_name']})
                 data_mgr[p_id]["big_data_email_sent"] = True
    ...
    try:
        while True:
            active_big_data_collect = 0 
            min_next_refresh = float('inf') 
            data_fetched_cycle = False 

            print("\n" + AnsiColors.BLUE + AnsiColors.BOLD + "─" * term_w + AnsiColors.ENDC)
            cycle_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            animated_text_display(f"🔄 New Cycle @ {cycle_time}", color=AnsiColors.BLUE, bold=True, center=True, delay=0.001, new_line=False)
            print(" " + AnsiColors.BLUE + AnsiColors.BOLD + "─" * term_w + AnsiColors.ENDC + "\n")

            for p_id, p_data in data_mgr.items():
                cfg = p_data["config"]
                p_name = cfg['pair_name'] 
                ...
                if p_data.get("data_fetch_failed_consecutively", 0) >= (key_manager.total_keys() or 1) + 1 : 
                    ...
                    if time_since_fail < 3600: # 1 jam cooldown
                        min_next_refresh = min(min_next_refresh, 3600 - time_since_fail)
                        continue 
                    else: # Cooldown selesai
                        p_data["data_fetch_failed_consecutively"] = 0 
                        log_info(f"Cooldown 1 jam {p_name} selesai. Coba fetch.", pair_name=p_name)

                time_now = datetime.now()
                secs_since_last_fetch = (time_now - p_data["last_candle_fetch_time"]).total_seconds()
                
                interval_needed = 0
                if p_data["big_data_collection_phase_active"]:
                    active_big_data_collect += 1
                    tf_intervals = {"minute": 55, "day": 3600 * 23.8, "hour": 3580}
                    interval_needed = tf_intervals.get(cfg['timeframe'], 3580)
                else: interval_needed = cfg['refresh_interval_seconds']
                
                if secs_since_last_fetch < interval_needed:
                    min_next_refresh = min(min_next_refresh, interval_needed - secs_since_last_fetch)
                    continue 

                log_info(f"🔎 Proses {AnsiColors.BOLD}{p_name}{AnsiColors.ENDC}...", pair_name=p_name)
                p_data["last_candle_fetch_time"] = time_now 
                candles_before = len(p_data["all_candles_list"])
                
                title_phase = f"--- ⏳ BIG DATA ({candles_before}/{TARGET_BIG_DATA_CANDLES}) ---" if p_data["big_data_collection_phase_active"] \
                              else f"--- 📊 ANALISA ({candles_before} candles) ---"
                ...
                new_batch, update_ok = [], False
                max_retries_update = key_manager.total_keys() or 1
                retries_update_done = 0
                ...
                while retries_update_done < max_retries_update and not update_ok:
                    ...
                    fetch_limit = 3 
                    if p_data["big_data_collection_phase_active"]:
                        needed_big = TARGET_BIG_DATA_CANDLES - candles_before
                        fetch_limit = min(needed_big, CRYPTOCOMPARE_MAX_LIMIT)
                        fetch_limit = max(fetch_limit, 1) # Ambil minimal 1 jika masih butuh
                        if needed_big <=0 : update_ok = True; break # Sudah terkumpul

                    log_info(f"Ambil {fetch_limit} candle (Key Idx: {key_manager.get_current_key_index()})...", pair_name=p_name)
                    try:
                        new_batch = fetch_candles(cfg['symbol'], cfg['currency'], fetch_limit, cfg['exchange'], key_for_attempt, cfg['timeframe'], pair_name=p_name)
                        update_ok = True
                        p_data["data_fetch_failed_consecutively"] = 0 
                        data_fetched_cycle = True 
                    except APIKeyError:
                        log_warning(f"API Key (Idx: {key_manager.get_current_key_index()}) gagal update {p_name}. Coba berikutnya.", pair_name=p_name)
                        p_data["data_fetch_failed_consecutively"] += 1
                        if not key_manager.switch_to_next_key(): log_error(f"Tidak ada API key global lagi setelah gagal {p_name}.", pair_name=p_name); break 
                        retries_update_done += 1 
                    except requests.exceptions.RequestException as e_req_upd:
                        log_error(f"Error jaringan update {p_name}: {e_req_upd}. Tidak ganti key.", pair_name=p_name)
                        p_data["data_fetch_failed_consecutively"] += 1; break 
                
                if p_data.get("data_fetch_failed_consecutively", 0) >= (key_manager.total_keys() or 1) +1 :
                    p_data["last_attempt_after_all_keys_failed"] = datetime.now() 
                    log_warning(f"Semua API key gagal untuk {p_name}. Masuk cooldown.", pair_name=p_name)

                if not update_ok or not new_batch:
                    if update_ok and not new_batch and not p_data["big_data_collection_phase_active"]:
                        log_warning(f"Tidak ada candle baru untuk {p_name} (fetch ok).", pair_name=p_name)
                    elif not update_ok: log_error(f"Gagal update {p_name} setelah semua upaya.", pair_name=p_name)
                    min_next_refresh = min(min_next_refresh, interval_needed)
                    print(AnsiColors.GREY + "-" * (term_w // 2) + AnsiColors.ENDC); continue 

                # Gabung candle
                merged_dict = {c['timestamp']: c for c in p_data["all_candles_list"]}
                ...
                p_data["all_candles_list"] = sorted(list(merged_dict.values()), key=lambda c_sort: c_sort['timestamp'])
                new_or_upd_total = added_count + updated_count
                ...
                # Cek Selesai Big Data
                if p_data["big_data_collection_phase_active"] and len(p_data["all_candles_list"]) >= TARGET_BIG_DATA_CANDLES:
                    log_info(f"{AnsiColors.GREEN}🎯 TARGET {TARGET_BIG_DATA_CANDLES} CANDLE TERCAPAI untuk {p_name}!{AnsiColors.ENDC}", pair_name=p_name)
                    if len(p_data["all_candles_list"]) > TARGET_BIG_DATA_CANDLES: 
                        p_data["all_candles_list"] = p_data["all_candles_list"][-TARGET_BIG_DATA_CANDLES:] 
                    if not p_data["big_data_email_sent"]:
                        send_email_notification(f"Data Complete: {p_name}", f"Download {TARGET_BIG_DATA_CANDLES} candles selesai! Trading aktif.", {**cfg, 'pair_name': p_name})
                        p_data["big_data_email_sent"] = True
                    p_data["big_data_collection_phase_active"] = False 
                    active_big_data_collect -=1 
                    log_info(f"{AnsiColors.HEADER}--- ▶️ MULAI LIVE ANALYSIS ({len(p_data['all_candles_list'])} candles) ---{AnsiColors.ENDC}", pair_name=p_name)
                elif not p_data["big_data_collection_phase_active"] and len(p_data["all_candles_list"]) > TARGET_BIG_DATA_CANDLES: 
                    p_data["all_candles_list"] = p_data["all_candles_list"][-TARGET_BIG_DATA_CANDLES:]
                ...
                if len(p_data["all_candles_list"]) >= min_len_piv_run:
                    run_logic = (new_or_upd_total > 0 or
                                 (not p_data["big_data_collection_phase_active"] and candles_before < TARGET_BIG_DATA_CANDLES and len(p_data["all_candles_list"]) >= TARGET_BIG_DATA_CANDLES) or
                                 (p_data["big_data_collection_phase_active"]))
                    if run_logic:
                         log_info(f"Jalankan strategi dengan {len(p_data['all_candles_list'])} candle...", pair_name=p_name)
                         p_data["strategy_state"] = run_strategy_logic(p_data["all_candles_list"], cfg, p_data["strategy_state"])
                         ...
                         log_info(f"Tidak ada candle baru untuk proses {p_name}. Data terakhir @ {last_c_ts}.", pair_name=p_name)
            ...
            # Durasi Sleep Global
            sleep_val = 15 
            if not data_fetched_cycle and key_manager.get_current_key() is None: 
                log_error("Semua API key gagal global & tidak ada data di-fetch. Tunggu 1 jam.", pair_name="SYSTEM")
                sleep_val = 3600 
                ...
                for pid_sleep, pdata_sleep in data_mgr.items():
                    if pdata_sleep["big_data_collection_phase_active"]:
                        cfg_sleep = pdata_sleep["config"]
                        tf_intervals_sleep = {"minute": 55, "day": 3600 * 23.8, "hour": 3580}
                        min_big_data_int = min(min_big_data_int, tf_intervals_sleep.get(cfg_sleep['timeframe'], 3580))
                sleep_val = min(min_big_data_int if min_big_data_int != float('inf') else 30, 30) # Max 30s jika masih big data
                log_debug(f"{active_big_data_collect} pair masih kumpulkan BIG DATA. Sleep {sleep_val}s.", pair_name="SYSTEM")
            else: 
                if min_next_refresh != float('inf') and min_next_refresh > 0:
                    sleep_val = max(MIN_REFRESH_INTERVAL_AFTER_BIG_DATA, int(min_next_refresh))
                    log_debug(f"Semua pair live. Tidur ~{sleep_val}s.", pair_name="SYSTEM")
                else: # Fallback
                    sleep_val = MIN_REFRESH_INTERVAL_AFTER_BIG_DATA 
                    log_debug(f"Default sleep {sleep_val}s (fallback).", pair_name="SYSTEM")
            
            if sleep_val > 0:
                show_spinner(sleep_val, f"💤 Menunggu {int(sleep_val)}s...", color=AnsiColors.BLUE)
        ...
        log_error(f"Error tak terduga loop utama: {e_loop}", pair_name="SYSTEM")
        log_exception("Traceback Error Loop Utama:", pair_name="SYSTEM")
    finally: 
        print("\n" + header_footer)
        animated_text_display("🏁 STRATEGY STOPPED 🏁", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002)
        print(header_footer)
        animated_text_display("Tekan Enter untuk kembali...", color=AnsiColors.ORANGE, bold=True, delay=0.01, center=True)
        input()
...
def main_menu():
    ...
    while True:
        clear_screen_animated()
        term_w_main = os.get_terminal_size().columns if os.isatty() else 60
        ...
        if active_cfgs_main:
            ...
            for i, c_main in enumerate(active_cfgs_main[:3]): # Tampilkan maks 3 untuk ringkasan
                title_main.append(f"    {AnsiColors.CYAN}{i+1}. {c_main['symbol']}-{c_main['currency']} {AnsiColors.GREY}({c_main['timeframe']}, {c_main['exchange']}){AnsiColors.ENDC}")
        ...
        try:
            ...
            if isinstance(e_main_menu, KeyboardInterrupt): 
                clear_screen_animated()
                animated_text_display(f"{AnsiColors.ORANGE}Aplikasi dihentikan. Bye!{AnsiColors.ENDC}", color=AnsiColors.ORANGE, bold=True, center=True); break
...
if __name__ == "__main__":
    try:
        clear_screen_animated()
        term_w_banner = os.get_terminal_size().columns if os.isatty() else 60
        banner_art = [
            "  ██████╗ ██████╗ ██╗   ██╗██████╗ ████████╗",
            "  ██╔══██╗██╔══██╗╚██╗ ██╔╝██╔══██╗╚══██╔══╝",
            "  ██████╔╝██████╔╝ ╚████╔╝ ██████╔╝   ██║   ",
        ...
        print(AnsiColors.CYAN + AnsiColors.BOLD) # Warna untuk banner
        for line_art in banner_art[:-1]: # Cetak ASCII Art
             print(line_art.center(term_w_banner)) # Center tanpa animasi untuk kecepatan
             time.sleep(0.03) # Jeda kecil antar baris
        ...
        import traceback
        traceback.print_exc()
        # Coba log jika logger sudah ada
        try:
            if logger and logger.handlers: # Pastikan logger ada dan punya handler
                logger.critical("MAIN LEVEL EXCEPTION (FATAL):", exc_info=True)
        except NameError: # Jika logger belum didefinisikan
            pass 
