...
def animated_text_display(text, delay=0.02, color=AnsiColors.CYAN, new_line=True, bold=False, center=False, width=None):
    ...
    if width is None:
        try:
            # Cek apakah output adalah TTY sebelum memanggil get_terminal_size
...
def simple_progress_bar(iteration, total, prefix='', suffix='', decimals=1, length=30, fill='❚', print_end="\r"):
    ...
    sys.stdout.write(f"\r{progress_line:{term_width_pb}}")
    sys.stdout.flush()
...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if not original_message.startswith('\033['): # Cek jika sudah ada ANSI code
...
def _prompt_crypto_config(current_config):
    ...
    try:
        if sys.stdout.isatty(): # Perbaikan di sini
            term_width_prompt = os.get_terminal_size().columns
    except OSError:
        pass
...
def settings_menu(current_settings):
    while True:
        ...
        term_width_settings = 60
        try:
            if sys.stdout.isatty(): # Perbaikan di sini
                term_width_settings = os.get_terminal_size().columns
        except OSError:
            pass
        
        title = [AnsiColors.HEADER + AnsiColors.BOLD + "╔" + "═" * (term_width_settings - 2) + "╗" + AnsiColors.ENDC,
                 AnsiColors.HEADER + AnsiColors.BOLD + "║" + "⚙️ PENGATURAN UTAMA".center(term_width_settings - 2) + "║" + AnsiColors.ENDC,
                 AnsiColors.HEADER + AnsiColors.BOLD + "╠" + "═" * (term_width_settings - 2) + "╣" + AnsiColors.ENDC]
        ...
        title.append(AnsiColors.GREY + "  " + "-" * (term_width_settings - 6) + AnsiColors.ENDC)
        ...
        title_pick += AnsiColors.GREY + "  " + "-" * (term_width_settings - 6) + AnsiColors.ENDC + "\n"
        ...
        try:
            if idx == 0: 
                ...
                api_s["primary_key"] = new_pk if new_pk else "YOUR_PRIMARY_KEY" 
                ...
                while True: 
                    ...
                    rec_title_lines = [AnsiColors.HEADER + AnsiColors.BOLD + "╔" + "═" * (term_width_settings - 2) + "╗" + AnsiColors.ENDC,
                                       AnsiColors.HEADER + AnsiColors.BOLD + "║" + "🔄 KELOLA RECOVERY API KEYS".center(term_width_settings - 2) + "║" + AnsiColors.ENDC,
                                       AnsiColors.HEADER + AnsiColors.BOLD + "╠" + "═" * (term_width_settings - 2) + "╣" + AnsiColors.ENDC]
                    ...
                    rec_title_lines.append(AnsiColors.GREY + "  " + "-" * (term_width_settings - 6) + AnsiColors.ENDC)
                ...
                print(AnsiColors.GREY + ("-" * term_width_settings) + AnsiColors.ENDC)
...
# ... (Fungsi fetch_candles, get_initial_strategy_state, find_pivots, run_strategy_logic SAMA seperti sebelumnya, tidak diubah karena tidak terkait error isatty)
# Untuk keringkasan, saya akan lewati pendefinisian ulang fungsi-fungsi ini di sini.
# Pastikan fungsi-fungsi ini ada di skrip Anda dari jawaban sebelumnya.
def fetch_candles(symbol, currency, total_limit_desired, exchange_name, current_api_key_to_use, timeframe="hour", pair_name="N/A"):
    ...
    if is_large or total_limit_desired <=5 : 
...
def run_strategy_logic(candles, cfg, state): 
    ...
    if state["final_pivot_high_price_confirmed"] is not None:
        ...
        if state["active_fib_level"] is not None: 
    ...
    if state["final_pivot_low_price_confirmed"] is not None and \
       ...
       idx_piv_event_l > state["high_bar_index_for_fib"]: 
    ...
    if state["active_fib_level"] is not None and state["position_size"] == 0 and \
       curr_c['close'] > curr_c['open'] and curr_c['close'] > state["active_fib_level"]:
        ...
        active_fib_level_for_log = state["active_fib_level"] # Simpan sebelum di-reset
        state.update({
        ...
        log_msg = f"🚀 BUY ENTRY @ {entry_px:.5f} (FIB {active_fib_level_for_log:.5f} dilewati). SL: {emerg_sl:.5f}" 
    ...
    if state["position_size"] > 0:
        ...
        current_high = curr_c.get('high', highest_trailing) 
        ...
        if stop_exit is not None and curr_low is not None and curr_low <= stop_exit:
            ...
            state.update(get_initial_strategy_state()) 
            state["last_signal_type"] = 0 
    ...
    if state["position_size"] > 0:
        ...
        if entry_p_log and curr_close_pnl != 0 and entry_p_log !=0: 
...
def start_trading(global_settings):
    ...
    try:
        if sys.stdout.isatty(): # Perbaikan di sini
            term_w_start = os.get_terminal_size().columns
    except OSError:
        pass
    header_footer = AnsiColors.HEADER + AnsiColors.BOLD + "═" * term_w_start + AnsiColors.ENDC
    print(header_footer)
    animated_text_display("🚀 MULTI-CRYPTO STRATEGY START 🚀", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002, width=term_w_start)
    print(header_footer)
    ...
    for cfg_item in active_cfgs:
        ...
        cfg_item['pair_name'] = f"{cfg_item['symbol']}-{cfg_item['currency']}" 
            ...
            if len(initial_data) >= min_len_piv:
                ...
                for i in range(min_len_piv -1, len(initial_data) - 1): 
                    ...
                    if data_mgr[p_id]["strategy_state"]["position_size"] > 0: 
                        ...
                        data_mgr[p_id]["strategy_state"]["last_signal_type"] = 0 
    ...
    try:
        while True:
            ...
            animated_text_display(f"🔄 New Cycle @ {cycle_time}", color=AnsiColors.BLUE, bold=True, center=True, delay=0.001, new_line=False, width=term_w_start)
            print(" " + AnsiColors.BLUE + AnsiColors.BOLD + "─" * term_w_start + AnsiColors.ENDC + "\n")
            ...
            for p_id, p_data in data_mgr.items():
                ...
                animated_text_display(title_phase, color=AnsiColors.MAGENTA if p_data["big_data_collection_phase_active"] else AnsiColors.CYAN, bold=True, delay=0.001, center=True, width=term_w_start)
                ...
                if not update_ok or not new_batch:
                    ...
                    print(AnsiColors.GREY + "-" * (term_w_start // 2) + AnsiColors.ENDC); continue 
            ...
            if not data_fetched_cycle and key_manager.get_current_key() is None: 
                ...
                sleep_val = min(min_big_data_int if min_big_data_int != float('inf') else 30, 30) 
        ...
        print("\n" + header_footer)
        animated_text_display("🏁 STRATEGY STOPPED 🏁", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002, width=term_w_start)
        print(header_footer)
        animated_text_display("Tekan Enter untuk kembali...", color=AnsiColors.ORANGE, bold=True, delay=0.01, center=True, width=term_w_start)
        input()
...
def main_menu():
    ...
    while True:
        ...
        title_main = [AnsiColors.HEADER + AnsiColors.BOLD + "╔" + "═" * (term_w_main_menu - 2) + "╗" + AnsiColors.ENDC,
                      AnsiColors.HEADER + AnsiColors.BOLD + "║" + "🤖 Crypto Strategy Runner 🤖".center(term_w_main_menu - 2) + "║" + AnsiColors.ENDC,
                      AnsiColors.HEADER + AnsiColors.BOLD + "╠" + "═" * (term_w_main_menu - 2) + "╣" + AnsiColors.ENDC]
        ...
        title_main.append(AnsiColors.GREY + "  " + "-" * (term_w_main_menu - 6) + AnsiColors.ENDC)
        ...
        try:
            ...
            if isinstance(e_main_menu, KeyboardInterrupt): 
                clear_screen_animated()
                animated_text_display(f"{AnsiColors.ORANGE}Aplikasi dihentikan. Bye!{AnsiColors.ENDC}", color=AnsiColors.ORANGE, bold=True, center=True, width=term_w_main_menu); break
...
if __name__ == "__main__":
    try:
        ...
        try:
            # Perbaikan krusial di sini: gunakan sys.stdout.isatty()
            if sys.stdout.isatty(): 
                term_w_banner_main = os.get_terminal_size().columns
        except OSError:
             # Biarkan term_w_banner_main tetap 60 jika gagal mendapatkan ukuran
            pass
        ...
        for line_art in banner_art[:-1]: 
             print(line_art.center(term_w_banner_main)) 
             time.sleep(0.03) 
        print(AnsiColors.ENDC)
        animated_text_display(banner_art[-1], color=AnsiColors.MAGENTA, bold=True, delay=0.005, center=True, width=term_w_banner_main)
        print("\n" * 1)
