...
def load_settings():
    ...
    default_settings = {
        ...
        "max_allowed_funding_rate_pct": 0.075, # BARU: Batas funding rate untuk entry
        "watched_pairs": {}
    }
...
def display_history():
    ...
    for trade in reversed(autopilot_trades):
        ...
        if trade['status'] == 'CLOSED':
            ...
            pl_percent_net = pl_percent_gross - fee_pct # DIUBAH: PnL bersih
...
def fetch_funding_rate(instId):
    # FUNGSI BARU: Mengambil funding rate saat ini dari Bybit
    ...
    try:
        ...
        if data.get("retCode") == 0 and 'list' in data.get('result', {}) and data['result']['list']:
            ...
            return float(funding_rate_str) * 100 # Convert to percentage
        else:
            # print_colored(f"API Error fetching funding rate for {instId}: {data.get('retMsg', 'Unknown')}", Fore.RED)
            return None
    except (requests.exceptions.RequestException, ValueError, KeyError) as e:
        # print_colored(f"Error fetching funding rate for {instId}: {e}", Fore.RED)
        return None
...
def fetch_recent_candles(instId, timeframe, limit=300):
    ...
    try:
        # Menggunakan kategori 'linear' untuk perpetual, 'spot' jika Anda trading spot
...
def calculate_winrate(trades_list, fee_pct):
    ...
    for trade in closed_trades:
        if trade.get('pl_percent', 0) - fee_pct > 0: # DIUBAH: Winrate berdasarkan PnL bersih
            profitable_trades += 1
        ...
        if trade.get('status') == 'CLOSED' and 'exitTimestamp' in trade:
            try:
                ...
                if exit_date == today_date:
                    total_pnl += (trade.get('pl_percent', 0.0) - fee_pct) # DIUBAH: PnL bersih
            except ValueError:
                continue
...
class LocalAI:
    ...
    def get_decision(self, candle_data, open_position, instrument_id, funding_rate=0.0):
        ...
        # DIUBAH: Logika AI untuk memeriksa Funding Rate
        ...
        if potential_trade_type:
            # Pemeriksaan Funding Rate sebelum validasi lainnya
            if potential_trade_type == 'LONG' and funding_rate > max_funding_rate:
                return {"action": "HOLD", "reason": f"HOLD LONG: Funding rate {funding_rate:.4f}% terlalu tinggi (batas: {max_funding_rate:.4f}%)"}
            if potential_trade_type == 'SHORT' and funding_rate < -max_funding_rate:
                return {"action": "HOLD", "reason": f"HOLD SHORT: Funding rate {funding_rate:.4f}% terlalu negatif (batas: {-max_funding_rate:.4f}%)"}
...
async def analyze_and_close_trade(trade, exit_price, close_trigger_reason, is_backtest=False):
    ...
    is_profit = (pnl - fee) > 0 # PnL bersih
    ...
    if not is_backtest:
        ...
        notif_content = f"PnL (Net): {pnl_net:.2f}% | Entry: {trade['entryPrice']:.4f} | Exit: {exit_price:.4f} | Trigger: {close_trigger_reason}"
...
async def run_autopilot_analysis(instrument_id):
    ...
    try:
        ...
        funding_rate = pair_state.get("funding_rate", 0.0) # DIUBAH: Ambil funding rate
        ...
        decision = local_brain.get_decision(candle_data, open_position, instrument_id, funding_rate) # DIUBAH: Kirim funding rate ke AI
    ...
    while not stop_event.is_set():
        ...
        if watched_pairs:
            for pair_id, timeframe in watched_pairs.items():
                if pair_id not in market_state:
                    market_state[pair_id] = {} # Inisialisasi jika belum ada
                ...
                if candle_data:
                    if len(candle_data) < 100 + 3:
                        print_colored(f"Warning: Not enough live candle data for {pair_id} to analyze ({len(candle_data)} candles). Skipping analysis.", Fore.YELLOW)
                        market_state[pair_id].update({"candle_data": candle_data, "analysis": None})
                        continue

                    analysis = LocalAI(current_settings, []).get_market_analysis(candle_data)
                    market_state[pair_id].update({"candle_data": candle_data, "analysis": analysis})
                    ...
                    if is_autopilot_running:
                        ...
                        if open_pos_for_management:
                            asyncio.run(check_realtime_position_management(open_pos_for_management, candle_data[-1]))
                time.sleep(0.5)
    ...
    while True:
        ...
        for i in range(total_candles_in_cumulative):
            ...
            if not any(t for t in temp_open_backtest_trades if t['instrumentId'] == pair_id):
                ...
                # Note: Backtest does not simulate funding rate, as it's not available in historical data
                decision = local_brain_for_decision.get_decision(current_historical_data_slice, None, pair_id, funding_rate=0.0)
                if decision.get('action') in ["BUY", "SELL"]:
                    trade_type = "LONG" if decision['action'] == "BUY" else "SHORT"
                    new_trade = {"id": int(current_candle['time'] / 1000), "instrumentId": pair_id, "type": trade_type, "entryTimestamp": datetime.fromtimestamp(current_candle['time'] / 1000).isoformat(), "entryPrice": current_candle['close'], "entryReason": decision.get("reason"), "status": 'OPEN', "entry_snapshot": decision.get("snapshot"), "run_up_percent": 0.0, "max_drawdown_percent": 0.0, "trailing_stop_price": None, "current_tp_checkpoint_level": 0.0}
        ...
        total_pnl = sum((t.get('pl_percent', 0.0) - current_settings.get('fee_pct', 0.1)) for t in trades_for_winrate_calc)
        num_trades = len(trades_for_winrate_calc)
        average_pnl = total_pnl / num_trades if num_trades > 0 else 0.0
        print_colored(f"\n  [DEBUG] Backtest Stats for {pair_id} ({num_trades} trades): Winrate: {current_cumulative_winrate:.2f}% | Avg P/L (Net): {average_pnl:.2f}% | Total P/L (Net): {total_pnl:.2f}% (Target: {target_winrate:.2f}%)", Fore.CYAN)
...
def handle_settings_command(parts):
    setting_map = {
        ...
        'fr_max': ('max_allowed_funding_rate_pct', '%') # BARU: Pengaturan funding rate
    }
    if len(parts) == 1 and parts[0] == '!settings':
        ...
        for key, (full_key, unit) in setting_map.items():
            ...
            display_value = current_settings.get(full_key, 'N/A')
            if full_key == 'cryptocompare_api_key' and display_value not in ["YOUR_CRYPTOCOMPARE_API_KEY", "N/A"]:
    ...
    if len(parts) == 3 and parts[0] == '!set':
        ...
        try:
            value = parts[2]
            if key_short in ['sl', 'fee', 'delay', 'tp_act', 'tp_gap', 'caution', 'winrate', 'fr_max']:
                value = float(value)
...
def run_dashboard_mode():
    try:
        while True:
            ...
            print_colored(f" | Today's P/L (Net): ", end="")
            print_colored(f"{todays_pnl:.2f}%", pnl_color_today, Style.BRIGHT, end="")
            print_colored(f" | This Week's P/L (Net): ", end="")
            print_colored(f"{this_weeks_pnl:.2f}%", pnl_color_week, Style.BRIGHT, end="")
            print_colored(f" | Last Week's P/L (Net): ", end="")
            print_colored(f"{last_weeks_pnl:.2f}%", pnl_color_last_week, Style.BRIGHT)
            ...
            for pair_id, timeframe in watched_pairs.items():
                ...
                if open_pos:
                    ...
                    pnl_gross = calculate_pnl(open_pos['entryPrice'], price, open_pos.get('type'))
                    pnl_net = pnl_gross - fee_pct
                    pnl_color_indiv = Fore.GREEN if pnl_net > 0 else Fore.RED
                    ...
                    print_colored("  PnL (Net) : ", end=''); print_colored(f"{pnl_net:.2f}%", pnl_color_indiv, Style.BRIGHT)
                    ...
                    if pair_state and pair_state.get("analysis"):
                        ...
                        funding_color = Fore.RED if funding_rate > 0 else Fore.GREEN if funding_rate < 0 else Fore.WHITE
                        bias_color = Fore.GREEN if analysis['bias'] == 'BULLISH' else Fore.RED if analysis['bias'] == 'BEARISH' else Fore.YELLOW
                        print_colored("  Trend     : ", end=''); print_colored(analysis['bias'], bias_color, end=" | ")
                        print_colored("Funding Rate: ", end=""); print_colored(f"{funding_rate:.4f}%", funding_color)
    ...
    while True:
        try:
            ...
            if cmd == '!exit': break
                ...
                else: print_colored("Format salah. Gunakan: !watch <PAIR> [TIMEFRAME] (e.g., !watch BTC-USDT 1H)", Fore.RED)
            elif cmd == '!unwatch':
                if len(command_parts) == 2:
                    pair_id = command_parts[1].upper()
