...
# ... (Fungsi animated_text_display, show_spinner, simple_progress_bar TETAP SAMA seperti di jawaban sebelumnya yang sudah benar untuk isatty)
def animated_text_display(text, delay=0.02, color=AnsiColors.CYAN, new_line=True, bold=False, center=False, width=None):
    ...
    for char_index, char in enumerate(text):
        ...
        sys.stdout.write(output_char); sys.stdout.flush(); time.sleep(delay)
    if new_line: print()
...
def show_spinner(duration_seconds, message="Processing...", color=AnsiColors.MAGENTA):
    ...
    while (time.time() - start_time) < duration_seconds:
        ...
        sys.stdout.write(f"\r{prefix}{display_message} {spinner_chars[idx % len(spinner_chars)]} {suffix}"); sys.stdout.flush()
        time.sleep(0.15); idx += 1
    sys.stdout.write(f"\r{' ' * (term_width -1)}\r"); sys.stdout.write(AnsiColors.ENDC); sys.stdout.flush()

def simple_progress_bar(iteration, total, prefix='', suffix='', decimals=1, length=30, fill='❚', print_end="\r"):
    term_width_pb = 80
    try:
        if sys.stdout.isatty():
            term_width_pb = os.get_terminal_size().columns
        non_bar_len = len(prefix.replace('\033[', '').split('m')[-1]) + \
                      len(suffix.replace('\033[', '').split('m')[-1]) + 12 
        length = max(10, min(length, term_width_pb - non_bar_len))
    except OSError: pass 
    ...
    sys.stdout.write(f"\r{progress_line:{term_width_pb}}"); sys.stdout.flush()
    if iteration == total and total > 0: sys.stdout.write('\n'); sys.stdout.flush()

# --- CUSTOM EXCEPTION ---
class APIKeyError(Exception): pass

# --- KONFIGURASI LOGGING ---
# ... (Konfigurasi logger TETAP SAMA)
...
console_formatter_template = (f"{AnsiColors.GREY}%(asctime)s{AnsiColors.ENDC} - %(log_color)s{AnsiColors.BOLD}%(levelname)-8s{AnsiColors.ENDC} - {AnsiColors.MAGENTA}[%(pair_name)s]{AnsiColors.ENDC} - %(log_color)s%(message_content)s{AnsiColors.ENDC}")
class ColoredFormatter(logging.Formatter):
    LOG_COLORS = {logging.DEBUG: AnsiColors.GREY, logging.INFO: AnsiColors.CYAN, logging.WARNING: AnsiColors.ORANGE, logging.ERROR: AnsiColors.RED, logging.CRITICAL: AnsiColors.BOLD + AnsiColors.RED,}
    def format(self, record):
        record.log_color = self.LOG_COLORS.get(record.levelno, AnsiColors.WHITE)
        level_emoji_map = {logging.INFO: "ℹ️ ", logging.WARNING: "⚠️ ", logging.ERROR: "❌ ", logging.CRITICAL: "🔥 "}
        original_message = record.getMessage()
        record.message_content = original_message
        if not original_message.startswith('\033['): record.message_content = level_emoji_map.get(record.levelno, "") + original_message
        formatter = logging.Formatter(console_formatter_template, datefmt='%H:%M:%S')
        return formatter.format(record)
ch.setFormatter(ColoredFormatter()); logger.addHandler(ch)
...
def clear_screen_animated(): os.system('cls' if os.name == 'nt' else 'clear')

# --- API KEY MANAGER ---
# ... (Kelas APIKeyManager TETAP SAMA)
class APIKeyManager:
    def __init__(self, primary_key, recovery_keys_list, global_settings_for_email=None):
        ...
        if recovery_keys_list: self.keys.extend([k for k in recovery_keys_list if isinstance(k, str) and k.strip()])
        self.current_index = 0
        self.global_email_settings = global_settings_for_email if global_settings_for_email is not None else {}
        if not self.keys: log_warning("Tidak ada API key yang valid (primary atau recovery) dikonfigurasi.")
    ...
    def switch_to_next_key(self):
        ...
        if self.current_index < len(self.keys):
            ...
            if self.global_email_settings.get("enable_global_email_notifications_for_key_switch", False):
                email_subject = "Peringatan: API Key CryptoCompare Diganti"; email_body = (f"Skrip trading otomatis mengganti API key CryptoCompare.\nKey sebelumnya mungkin limit/tidak valid.\nSekarang menggunakan API key index: {self.current_index}\nKey: ...{new_key_display[-8:]}\nPeriksa status API key Anda di CryptoCompare.")
                cfg_for_email = {"enable_email_notifications": True, "email_sender_address": self.global_email_settings.get("email_sender_address"), "email_sender_app_password": self.global_email_settings.get("email_sender_app_password"), "email_receiver_address": self.global_email_settings.get("email_receiver_address_admin", self.global_email_settings.get("email_receiver_address"))}
                if all(cfg_for_email.get(k) for k in ["email_sender_address", "email_sender_app_password", "email_receiver_address"]): send_email_notification(email_subject, email_body, cfg_for_email, pair_name_override="SYSTEM_KEY_SWITCH")
                else: log_warning("Konfigurasi email global tidak lengkap untuk notifikasi ganti API key.")
            return new_key
        else:
            log_error(f"{AnsiColors.RED}{AnsiColors.BOLD}🆘 SEMUA API KEY HABIS/GAGAL! Tidak dapat ambil data.{AnsiColors.ENDC}")
            if self.global_email_settings.get("enable_global_email_notifications_for_key_switch", False):
                email_subject = "KRITIS: SEMUA API Key CryptoCompare Gagal!"; email_body = (f"Skrip trading telah mencoba semua API key CryptoCompare dan semuanya gagal.\nSkrip tidak dapat lagi mengambil data harga.\nSegera periksa akun CryptoCompare Anda dan konfigurasi API key.")
                cfg_for_email_critical = {"enable_email_notifications": True, "email_sender_address": self.global_email_settings.get("email_sender_address"), "email_sender_app_password": self.global_email_settings.get("email_sender_app_password"), "email_receiver_address": self.global_email_settings.get("email_receiver_address_admin", self.global_email_settings.get("email_receiver_address"))}
                if all(cfg_for_email_critical.get(k) for k in ["email_sender_address", "email_sender_app_password", "email_receiver_address"]): send_email_notification(email_subject, email_body, cfg_for_email_critical, pair_name_override="SYSTEM_KEY_FAIL")
                else: log_warning("Konfigurasi email global tidak lengkap untuk notifikasi KRITIS API key habis.")
            return None
...
# --- FUNGSI BEEP & EMAIL ---
# ... (Fungsi play_notification_sound, send_email_notification TETAP SAMA)
def play_notification_sound(count=1, interval=0.15, freq=1000, dur=200):
    try:
        for _ in range(count):
            if sys.platform == "win32": import winsound; winsound.Beep(freq, dur) 
...
def send_email_notification(subject, body_text, settings_for_email, pair_name_override=None):
    ...
    sender = settings_for_email.get("email_sender_address"); password = settings_for_email.get("email_sender_app_password"); receiver = settings_for_email.get("email_receiver_address")
    ...
    if not all([sender, password, receiver]): log_warning(f"Konfigurasi email tidak lengkap. Notifikasi email dilewati.", pair_name=ctx_pair_name); return
    msg = MIMEText(body_text); msg['Subject'] = f"📢 {subject}"; msg['From'] = sender; msg['To'] = receiver
    try:
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as server: server.login(sender, password); server.sendmail(sender, receiver, msg.as_string())
...
def load_settings():
    ...
    if os.path.exists(SETTINGS_FILE):
        try:
            ...
            for cfg in settings["cryptos"]: cfg.setdefault("id", str(uuid.uuid4())); cfg.setdefault("enabled", True)
            return settings
        except json.JSONDecodeError: log_error(f"Error baca {SETTINGS_FILE}. Membuat file default baru."); return {"api_settings": defaults.copy(), "cryptos": []}
        except Exception as e: log_error(f"Error load_settings: {e}. Menggunakan default."); return {"api_settings": defaults.copy(), "cryptos": [get_default_crypto_config()]} 
    else: log_info(f"File {SETTINGS_FILE} tidak ditemukan. Membuat dengan default."); new_settings = {"api_settings": defaults.copy(), "cryptos": []}; save_settings(new_settings); return new_settings
def save_settings(settings):
    ...
    except Exception as e: log_error(f"Gagal menyimpan pengaturan ke {SETTINGS_FILE}: {e}")
def termux_input(prompt_text, current_value="", color=AnsiColors.CYAN, highlight_color=AnsiColors.WHITE, is_password=False, prefix_icon="▸"):
    prompt_prefix = f"{color}{AnsiColors.BOLD}{prefix_icon} {prompt_text}{AnsiColors.ENDC}"; default_suffix = f" [{AnsiColors.ORANGE}{current_value}{AnsiColors.ENDC}]" if current_value or isinstance(current_value, (bool, int, float)) else ""; input_indicator = f": {highlight_color}"; full_prompt = f"{prompt_prefix}{default_suffix}{input_indicator}"
    user_input = "";
    if is_password:
        try: 
            if 'getpass' in sys.modules: user_input = getpass.getpass(prompt=full_prompt)
            else: raise ImportError 
        except (ImportError, RuntimeError, EOFError): print(full_prompt, end=""); sys.stdout.flush(); user_input = sys.stdin.readline().strip()
    else: print(full_prompt, end=""); sys.stdout.flush(); user_input = sys.stdin.readline().strip()
    sys.stdout.write(AnsiColors.ENDC); return user_input or str(current_value) 
def _prompt_crypto_config(current_config):
    clear_screen_animated(); new_cfg = current_config.copy(); sym = new_cfg.get('symbol','BARU'); cur = new_cfg.get('currency','BARU')
    ...
    term_width_prompt = 60
    try:
        if sys.stdout.isatty(): term_width_prompt = os.get_terminal_size().columns
    except OSError: pass
    print(AnsiColors.GREY + ("-" * term_width_prompt) + AnsiColors.ENDC)
    en_raw = termux_input("Aktifkan pair ini?", str(new_cfg.get('enabled',True)).lower()).lower()
    new_cfg["enabled"] = True if en_raw == 'true' else (False if en_raw == 'false' else new_cfg.get('enabled',True)); print()
    ...
    new_cfg["exchange"] = termux_input("Exchange (Binance, CCCAGG)", new_cfg.get('exchange','CCCAGG')); print()
    ...
    except ValueError: log_warning(f"Input refresh interval tidak valid, menggunakan default."); print()
    animated_text_display("-- Parameter Pivot --", color=AnsiColors.HEADER, bold=True, delay=0.005)
    try: new_cfg["left_strength"] = int(termux_input("Left Strength", str(new_cfg.get('left_strength',50)))); new_cfg["right_strength"] = int(termux_input("Right Strength", str(new_cfg.get('right_strength',150))))
    except ValueError: log_warning(f"Input strength tidak valid, menggunakan default."); print()
    animated_text_display("-- Parameter Trading --", color=AnsiColors.HEADER, bold=True, delay=0.005)
    try: new_cfg["profit_target_percent_activation"] = float(termux_input("Profit % Aktivasi Trailing TP", str(new_cfg.get('profit_target_percent_activation',5.0)))); new_cfg["trailing_stop_gap_percent"] = float(termux_input("Gap Trailing TP %", str(new_cfg.get('trailing_stop_gap_percent',5.0)))); new_cfg["emergency_sl_percent"] = float(termux_input("Emergency SL %", str(new_cfg.get('emergency_sl_percent',10.0)), color=AnsiColors.ORANGE))
    except ValueError: log_warning(f"Input parameter trading tidak valid, menggunakan default."); print()
    ...
    else: log_warning(f"Pilihan harga Secure FIB tidak valid, menggunakan '{new_cfg.get('secure_fib_check_price','Close')}'"); print()
    ...
    new_cfg["email_receiver_address"] = termux_input("Email Penerima", new_cfg.get('email_receiver_address','')); print()
    return new_cfg
...
def settings_menu(current_settings):
    while True:
        ...
        # BUAT TITLE PLAIN UNTUK PICK
        plain_title_settings = "--- PENGATURAN UTAMA ---\n"
        plain_title_settings += f"Primary API Key : {pk_disp}\n"
        plain_title_settings += f"Recovery Keys   : {num_rec_keys} tersimpan\n"
        ...
        plain_title_settings += f"Email Global Sys: {g_email_stat}\n"
        plain_title_settings += "--------------------------\n"
        plain_title_settings += "Daftar Konfigurasi Crypto:\n"
        if not current_settings["cryptos"]:
            plain_title_settings += "  (Belum ada konfigurasi crypto)\n"
        for i, cfg in enumerate(current_settings["cryptos"]):
            stat_txt = "Aktif" if cfg.get('enabled', True) else "Nonaktif"
            plain_title_settings += f"  {i+1}. {cfg['symbol']}-{cfg['currency']} ({cfg['timeframe']}) - {stat_txt}\n"
        plain_title_settings += "--------------------------\n"
        plain_title_settings += "Pilih tindakan:"
        ...
        try:
            # GUNAKAN TITLE PLAIN DAN INDICATOR PLAIN
            opt_txt, idx = pick(options, plain_title_settings, indicator='=> ', default_index=0)
        ...
        try:
            if idx == 0: 
                ...
                while True: 
                    clear_screen_animated()
                    api_s_rec = current_settings.get("api_settings", {}) 
                    current_rec_keys = [k for k in api_s_rec.get('recovery_keys', []) if isinstance(k, str) and k.strip()]
                    
                    # BUAT TITLE PLAIN UNTUK SUB-MENU RECOVERY
                    plain_rec_title = "--- KELOLA RECOVERY API KEYS ---\n"
                    if not current_rec_keys: plain_rec_title += "  (Tidak ada recovery key tersimpan)\n"
                    else:
                        for i, rk in enumerate(current_rec_keys):
                            rk_disp = rk[:5] + "..." + rk[-3:] if len(rk) > 8 else rk
                            plain_rec_title += f"  {i+1}. {rk_disp}\n"
                    plain_rec_title += "--------------------------\n"
                    plain_rec_title += "Pilih tindakan:"
                    
                    rec_options = ["➕ Tambah Recovery Key", "🗑️ Hapus Recovery Key", "🚪 Kembali"]
                    try:
                        # GUNAKAN TITLE PLAIN DAN INDICATOR PLAIN
                        rec_opt_text, rec_idx = pick(rec_options, plain_rec_title, indicator='=> ')
                    ...
                    if rec_idx == 0: 
                        ...
                        if new_rk_val and new_rk_val not in current_rec_keys:
                            current_rec_keys.append(new_rk_val); api_s_rec['recovery_keys'] = current_rec_keys; current_settings['api_settings']['recovery_keys'] = current_rec_keys
                            save_settings(current_settings); log_info("Recovery key ditambahkan.")
                        ...
                        if not current_rec_keys: log_warning("Tidak ada recovery key untuk dihapus.")
                            ...
                            del_title_rk = "Pilih key yang akan dihapus:" # PLAIN TITLE
                            try:
                                _, del_idx_rk = pick(del_opts_rk, del_title_rk, indicator='=> ') # PLAIN INDICATOR
                                if del_idx_rk < len(current_rec_keys): 
                                    removed_rk = current_rec_keys.pop(del_idx_rk); api_s_rec['recovery_keys'] = current_rec_keys; current_settings['api_settings']['recovery_keys'] = current_rec_keys
                                    save_settings(current_settings); log_info(f"Recovery key '{removed_rk[:5]}...' dihapus.")
                                ...
                                else: log_warning(f"Error pick hapus recovery key: {e_del_rk_pick}"); show_spinner(1, "Kembali...")
                    elif rec_idx == 2: break 
                    if rec_idx in [0,1]: show_spinner(1.5, "Memproses...", color=AnsiColors.BLUE)
            elif idx == 2: 
                animated_text_display("-- 📧 Pengaturan Email Global --", color=AnsiColors.HEADER, bold=True, center=True); print(AnsiColors.GREY + "Email untuk notifikasi sistem (ganti API key, dll)." + AnsiColors.ENDC)
                term_width_s = 60; try: term_width_s = os.get_terminal_size().columns if sys.stdout.isatty() else 60 except OSError: pass
                print(AnsiColors.GREY + ("-" * term_width_s) + AnsiColors.ENDC)
                ...
                new_crypto = _prompt_crypto_config(get_default_crypto_config()); current_settings["cryptos"].append(new_crypto); save_settings(current_settings); log_info(f"Konfigurasi {new_crypto['symbol']}-{new_crypto['currency']} ditambahkan.")
            elif idx == 4: 
                if not current_settings["cryptos"]: log_warning("Tidak ada konfigurasi untuk diubah.")
                    ...
                    edit_title = "Pilih crypto yang akan diubah:" # PLAIN
                    try:
                        _, idx_edit = pick(crypto_opts_edit, edit_title, indicator='=> ') # PLAIN
                        if idx_edit < len(current_settings["cryptos"]):
                            current_settings["cryptos"][idx_edit] = _prompt_crypto_config(current_settings["cryptos"][idx_edit]); save_settings(current_settings); log_info(f"Konfigurasi {current_settings['cryptos'][idx_edit]['symbol']}-{current_settings['cryptos'][idx_edit]['currency']} diubah.")
                        ...
                        else: log_warning(f"Error pick ubah crypto: {e_edit_c_pick}"); show_spinner(1, "Kembali...")
            elif idx == 5: 
                if not current_settings["cryptos"]: log_warning("Tidak ada konfigurasi untuk dihapus.")
                    ...
                    del_title = "Pilih crypto yang akan dihapus:" # PLAIN
                    try:
                        _, idx_del = pick(crypto_opts_del, del_title, indicator='=> ') # PLAIN
                        if idx_del < len(current_settings["cryptos"]):
                            removed = current_settings["cryptos"].pop(idx_del); save_settings(current_settings); log_info(f"Konfigurasi {removed['symbol']}-{removed['currency']} dihapus.")
                        else: log_info("Penghapusan dibatalkan.")
                        ...
                        else: log_warning(f"Error pick hapus crypto: {e_del_c_pick}"); show_spinner(1, "Kembali...")
            elif idx == 6: break 
        ...
        except Exception as e_menu_action: log_error(f"Error di menu pengaturan: {e_menu_action}"); log_exception("Traceback Pengaturan:"); show_spinner(1.5, "Error besar, kembali...", color=AnsiColors.RED)
    return current_settings

# --- FUNGSI PENGAMBILAN DATA, STRATEGI, LOOP TRADING UTAMA ---
# ... (Semua fungsi ini: fetch_candles, get_initial_strategy_state, find_pivots, run_strategy_logic, start_trading TETAP SAMA seperti di jawaban SEBELUMNYA YANG SUDAH FIX isatty error)
# Saya akan lewati pendefinisian ulang di sini demi keringkasan. PASTIKAN Anda menggunakan versi dari jawaban sebelumnya yang sudah memperbaiki 'isatty'.
# Contoh:
def fetch_candles(symbol, currency, total_limit_desired, exchange_name, current_api_key_to_use, timeframe="hour", pair_name="N/A"):
    ...
    if not current_api_key_to_use: log_error(f"Tidak ada API key untuk fetch_candles.", pair_name=pair_name); raise APIKeyError("API Key tidak tersedia.")
    all_candles, to_ts = [], None; endpoint = {"minute": "histominute", "hour": "histohour", "day": "histoday"}.get(timeframe, "histohour")
    url = f"https://min-api.cryptocompare.com/data/v2/{endpoint}"; is_large = total_limit_desired > 20
    if is_large and total_limit_desired > CRYPTOCOMPARE_MAX_LIMIT: log_info(f"Pengambilan data besar: target {total_limit_desired} TF {timeframe}.", pair_name=pair_name); simple_progress_bar(0, total_limit_desired, prefix=f'{AnsiColors.BOLD}{pair_name}{AnsiColors.ENDC} Data:', suffix='Candles', length=20)
    loops = 0
    while len(all_candles) < total_limit_desired:
        needed = total_limit_desired - len(all_candles); limit = min(needed + 1 if to_ts and needed > 1 else needed, CRYPTOCOMPARE_MAX_LIMIT)
        if limit <= 0: break
        ...
        try:
            ...
            if resp.status_code in [401, 403, 429]: err_data = resp.json() if resp.content else {}; err_msg = err_data.get('Message', f"HTTP {resp.status_code}"); log_warning(f"API Key Error (HTTP {resp.status_code}): {err_msg} Key: ...{key_short}", pair_name=pair_name); raise APIKeyError(f"HTTP {resp.status_code}: {err_msg}")
            ...
            if data.get('Response') == 'Error':
                ...
                if any(k_err.lower() in err_msg.lower() for k_err in key_errs): log_warning(f"API Key Error (JSON): {err_msg} Key: ...{key_short}", pair_name=pair_name); raise APIKeyError(f"JSON Error: {err_msg}")
            ...
            loops +=1
            if is_large and total_limit_desired > CRYPTOCOMPARE_MAX_LIMIT and (loops % 2 == 0 or len(all_candles) >= total_limit_desired): simple_progress_bar(len(all_candles), total_limit_desired, prefix=f'{AnsiColors.BOLD}{pair_name}{AnsiColors.ENDC} Data:', suffix='Candles', length=20)
...
def find_pivots(series_list, left_strength, right_strength, is_high=True):
    ...
    for i in range(left_strength, len(series_list) - right_strength):
        ...
        is_pivot = True
        for j in range(1, left_strength + 1): val_ij = series_list[i-j]; 
        ...
        for j in range(1, right_strength + 1): val_ij = series_list[i+j]; 
            if val_ij is None: is_pivot = False; break; 
            if (is_high and val_i < val_ij) or (not is_high and val_i > val_ij): is_pivot = False; break
...
def run_strategy_logic(candles, cfg, state): 
    ...
    raw_piv_h_price = piv_highs[idx_piv_event_h] if 0 <= idx_piv_event_h < len(piv_highs) else None; raw_piv_l_price = piv_lows[idx_piv_event_l] if 0 <= idx_piv_event_l < len(piv_lows) else None
    if raw_piv_h_price is not None and state["last_signal_type"] != 1:
        ...
        if 0 <= idx_piv_event_h < len(candles) and candles[idx_piv_event_h] and candles[idx_piv_event_h].get('timestamp'): ts = candles[idx_piv_event_h]['timestamp']; log_info(f"{AnsiColors.CYAN}📈 PIVOT HIGH: {state['final_pivot_high_price_confirmed']:.5f} @ {ts.strftime('%H:%M')}{AnsiColors.ENDC}", pair_name=pair)
    ...
    if raw_piv_l_price is not None and state["last_signal_type"] != -1:
        ...
        if 0 <= idx_piv_event_l < len(candles) and candles[idx_piv_event_l] and candles[idx_piv_event_l].get('timestamp'): ts = candles[idx_piv_event_l]['timestamp']; log_info(f"{AnsiColors.CYAN}📉 PIVOT LOW:  {state['final_pivot_low_price_confirmed']:.5f} @ {ts.strftime('%H:%M')}{AnsiColors.ENDC}", pair_name=pair)
        else: log_warning(f"Indeks pivot low ({idx_piv_event_l}) di luar jangkauan/data invalid.", pair_name=pair)
    curr_c = candles[curr_idx] 
    if not curr_c or any(curr_c.get(k) is None for k in ['open', 'high', 'low', 'close']): ts_str = curr_c.get('timestamp', datetime.now()).strftime('%H:%M'); log_warning(f"Data OHLC tidak lengkap untuk candle terbaru @ {ts_str}. Skip.", pair_name=pair); return state
    ...
    if state["active_fib_level"] is not None and state["position_size"] == 0 and curr_c['close'] > curr_c['open'] and curr_c['close'] > state["active_fib_level"]:
        entry_px = curr_c['close']; emerg_sl = entry_px * (1 - cfg["emergency_sl_percent"] / 100.0); active_fib_level_for_log = state["active_fib_level"] 
        ...
        log_msg = f"🚀 BUY ENTRY @ {entry_px:.5f} (FIB {active_fib_level_for_log:.5f} dilewati). SL: {emerg_sl:.5f}"; log_info(f"{AnsiColors.GREEN}{AnsiColors.BOLD}{log_msg}{AnsiColors.ENDC}", pair_name=pair); play_notification_sound(count=2, freq=1200, dur=150)
        ts_email = curr_c['timestamp'].strftime('%Y-%m-%d %H:%M:%S'); send_email_notification(f"BUY Signal: {pair}", f"New BUY: {pair} @ {entry_px:.5f}\nFIB Lvl: {active_fib_level_for_log:.5f}\nEmerg SL: {emerg_sl:.5f}\nTime: {ts_email}", {**cfg, 'pair_name': pair}) 
    if state["position_size"] > 0:
        highest_trailing = state.get("highest_price_for_trailing", curr_c.get('high', 0)); current_high = curr_c.get('high', highest_trailing); state["highest_price_for_trailing"] = max(highest_trailing, current_high)
...
def start_trading(global_settings):
    clear_screen_animated(); api_cfg = global_settings.get("api_settings", {}); key_manager = APIKeyManager(api_cfg.get("primary_key"), api_cfg.get("recovery_keys", []), api_cfg)
    ...
    term_w_start = 60; 
    try: term_w_start = os.get_terminal_size().columns if sys.stdout.isatty() else 60
    except OSError: pass
    header_footer = AnsiColors.HEADER + AnsiColors.BOLD + "═" * term_w_start + AnsiColors.ENDC; print(header_footer); animated_text_display("🚀 MULTI-CRYPTO STRATEGY START 🚀", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002, width=term_w_start); print(header_footer)
    ...
    for cfg_item in active_cfgs:
        ...
        animated_text_display(f"\n✨ Init: {AnsiColors.BOLD}{cfg_item['pair_name']}{AnsiColors.ENDC} | {cfg_item['exchange']} | TF: {cfg_item['timeframe']}", color=AnsiColors.MAGENTA, delay=0.003, width=term_w_start)
        ...
        while retries_init < max_retries_init_val and not fetched_ok:
            ...
            try: log_info(f"BIG DATA: Ambil {target_init} candle (Key Idx: {key_manager.get_current_key_index()})...", pair_name=cfg_item['pair_name']); initial_data = fetch_candles(cfg_item['symbol'], cfg_item['currency'], target_init, cfg_item['exchange'], curr_key, cfg_item['timeframe'], pair_name=cfg_item['pair_name']); fetched_ok = True
            except APIKeyError: log_warning(f"BIG DATA: API Key gagal untuk {cfg_item['pair_name']}. Coba berikutnya.", pair_name=cfg_item['pair_name']); 
        ...
        if initial_data:
            ...
            if len(initial_data) >= min_len_piv:
                ...
                for i in range(min_len_piv -1, len(initial_data) - 1): 
                    ...
                    temp_state = data_mgr[p_id]["strategy_state"].copy(); temp_state["position_size"] = 0; data_mgr[p_id]["strategy_state"] = run_strategy_logic(hist_slice, cfg_item, temp_state)
                    if data_mgr[p_id]["strategy_state"]["position_size"] > 0: data_mgr[p_id]["strategy_state"].update(get_initial_strategy_state()); data_mgr[p_id]["strategy_state"]["last_signal_type"] = 0 
        ...
        print(AnsiColors.GREY + "-" * (term_w_start // 2) + AnsiColors.ENDC)
    try:
        while True:
            active_big_data_collect = 0; min_next_refresh = float('inf'); data_fetched_cycle = False 
            print("\n" + AnsiColors.BLUE + AnsiColors.BOLD + "─" * term_w_start + AnsiColors.ENDC); cycle_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S'); animated_text_display(f"🔄 New Cycle @ {cycle_time}", color=AnsiColors.BLUE, bold=True, center=True, delay=0.001, new_line=False, width=term_w_start); print(" " + AnsiColors.BLUE + AnsiColors.BOLD + "─" * term_w_start + AnsiColors.ENDC + "\n")
            for p_id, p_data in data_mgr.items():
                ...
                title_phase = f"--- ⏳ BIG DATA ({candles_before}/{TARGET_BIG_DATA_CANDLES}) ---" if p_data["big_data_collection_phase_active"] else f"--- 📊 ANALISA ({candles_before} candles) ---"; animated_text_display(title_phase, color=AnsiColors.MAGENTA if p_data["big_data_collection_phase_active"] else AnsiColors.CYAN, bold=True, delay=0.001, center=True, width=term_w_start)
                ...
                if not update_ok or not new_batch:
                    if update_ok and not new_batch and not p_data["big_data_collection_phase_active"]: log_warning(f"Tidak ada candle baru untuk {p_name} (fetch ok).", pair_name=p_name)
                    elif not update_ok: log_error(f"Gagal update {p_name} setelah semua upaya.", pair_name=p_name)
                    min_next_refresh = min(min_next_refresh, interval_needed); print(AnsiColors.GREY + "-" * (term_w_start // 2) + AnsiColors.ENDC); continue 
                merged_dict = {c['timestamp']: c for c in p_data["all_candles_list"]}; added_count, updated_count = 0,0
                for c_new in new_batch: ts_new = c_new['timestamp']; 
                ...
                if len(p_data["all_candles_list"]) >= min_len_piv_run:
                    ...
                    if run_logic: log_info(f"Jalankan strategi dengan {len(p_data['all_candles_list'])} candle...", pair_name=p_name); p_data["strategy_state"] = run_strategy_logic(p_data["all_candles_list"], cfg, p_data["strategy_state"])
                    elif not p_data["big_data_collection_phase_active"]: last_c_ts = p_data["all_candles_list"][-1]['timestamp'].strftime('%H:%M:%S') if p_data["all_candles_list"] and p_data["all_candles_list"][-1] and p_data["all_candles_list"][-1].get('timestamp') else "N/A"; log_info(f"Tidak ada candle baru untuk proses {p_name}. Data terakhir @ {last_c_ts}.", pair_name=p_name)
                else: log_info(f"Data ({len(p_data['all_candles_list'])}) {p_name} belum cukup (min: {min_len_piv_run}).", pair_name=p_name)
                min_next_refresh = min(min_next_refresh, interval_needed); print(AnsiColors.GREY + "-" * (term_w_start // 2) + AnsiColors.ENDC) 
            sleep_val = 15 
    ...
    except KeyboardInterrupt: animated_text_display(f"\n{AnsiColors.ORANGE}🛑 Proses dihentikan pengguna.{AnsiColors.ENDC}", color=AnsiColors.ORANGE, bold=True, delay=0.01, center=True, width=term_w_start)
    ...
    finally: print("\n" + header_footer); animated_text_display("🏁 STRATEGY STOPPED 🏁", color=AnsiColors.HEADER, bold=True, center=True, delay=0.002, width=term_w_start); print(header_footer); animated_text_display("Tekan Enter untuk kembali...", color=AnsiColors.ORANGE, bold=True, delay=0.01, center=True, width=term_w_start); input()

# --- MENU UTAMA ---
def main_menu():
    settings = load_settings()
    while True:
        clear_screen_animated() # Ini akan membersihkan layar
        
        # Tampilkan judul menu utama DENGAN WARNA sebelum memanggil pick
        term_w_main_menu = 60
        try:
            if sys.stdout.isatty(): term_w_main_menu = os.get_terminal_size().columns
        except OSError: pass
        
        print(AnsiColors.HEADER + AnsiColors.BOLD + "╔" + "═" * (term_w_main_menu - 2) + "╗" + AnsiColors.ENDC)
        print(AnsiColors.HEADER + AnsiColors.BOLD + "║" + "🤖 Crypto Strategy Runner 🤖".center(term_w_main_menu - 2) + "║" + AnsiColors.ENDC)
        print(AnsiColors.HEADER + AnsiColors.BOLD + "╠" + "═" * (term_w_main_menu - 2) + "╣" + AnsiColors.ENDC)
        
        # BUATLAH STRING `pick_title` YANG PLAIN (TANPA ANSI) UNTUK `pick`
        plain_pick_title = "" # Mulai kosong, atau dengan header minimal jika perlu
        
        active_cfgs_main = [c for c in settings.get("cryptos", []) if c.get("enabled", True)]
        if active_cfgs_main:
            # Cetak info ini DENGAN WARNA sebelum pick
            print(AnsiColors.GREEN + AnsiColors.BOLD + f"  Crypto Aktif ({len(active_cfgs_main)}):" + AnsiColors.ENDC)
            for i, c_main in enumerate(active_cfgs_main[:3]):
                print(f"    {AnsiColors.CYAN}{i+1}. {c_main['symbol']}-{c_main['currency']} {AnsiColors.GREY}({c_main['timeframe']}, {c_main['exchange']}){AnsiColors.ENDC}")
            if len(active_cfgs_main) > 3: print(f"    {AnsiColors.GREY}...dan {len(active_cfgs_main)-3} lainnya.{AnsiColors.ENDC}")
        else:
            print(AnsiColors.ORANGE + "  Tidak ada konfigurasi crypto aktif." + AnsiColors.ENDC)
        
        print(AnsiColors.GREY + "  " + "-" * (term_w_main_menu - 6) + AnsiColors.ENDC)
        ...
        print(f"  {AnsiColors.BLUE}Target Data: {AnsiColors.WHITE}{TARGET_BIG_DATA_CANDLES} c/pair{AnsiColors.ENDC}")
        print(f"  {AnsiColors.BLUE}Primary Key: {pk_clr_main}{pk_disp_main}{AnsiColors.ENDC} | {AnsiColors.BLUE}Recovery: {rk_clr_main}{num_rec_keys_main}{AnsiColors.ENDC}")
        print(AnsiColors.GREY + "  " + "-" * (term_w_main_menu - 6) + AnsiColors.ENDC)
        
        # Title untuk pick sekarang bisa lebih sederhana atau bahkan string kosong
        # karena info penting sudah dicetak di atas.
        plain_pick_title = "Pilih Opsi:" # Atau bisa juga string kosong jika mau: ""

        options_main = ["🚀 Mulai Analisa Realtime", "⚙️  Pengaturan", "🚪 Keluar"]
        
        try:
            # Gunakan indicator polosan juga
            opt_txt_main, idx_main = pick(options_main, plain_pick_title, indicator='=> ') 
            ...
            if idx_main == 0: start_trading(settings)
                ...
                animated_text_display("👋 Terima kasih! Sampai jumpa! 👋", color=AnsiColors.MAGENTA, bold=True, center=True, delay=0.01, width=term_w_main_menu)
            ...
            if isinstance(e_main_menu, KeyboardInterrupt): 
                clear_screen_animated(); animated_text_display(f"{AnsiColors.ORANGE}Aplikasi dihentikan. Bye!{AnsiColors.ENDC}", color=AnsiColors.ORANGE, bold=True, center=True, width=term_w_main_menu); break
            show_spinner(1.5, "Error menu, coba lagi...", color=AnsiColors.RED)

if __name__ == "__main__":
    try:
        clear_screen_animated(); term_w_banner_main = 60
        try: term_w_banner_main = os.get_terminal_size().columns if sys.stdout.isatty() else 60
        except OSError: pass
        banner_art = ["  ██████╗ ██████╗ ██╗   ██╗██████╗ ████████╗","  ██╔══██╗██╔══██╗╚██╗ ██╔╝██╔══██╗╚══██╔══╝","  ██████╔╝██████╔╝ ╚████╔╝ ██████╔╝   ██║   ","  ██╔═══╝ ██╔══██╗  ╚██╔╝  ██╔═══╝    ██║   ","  ██║     ██║  ██║   ██║   ██║        ██║   ","  ╚═╝     ╚═╝  ╚═╝   ╚═╝   ╚═╝        ╚═╝   ","  CRYPTOCURRENCY TRADING BOT (Enhanced UI)",]
        print(AnsiColors.CYAN + AnsiColors.BOLD) 
        for line_art in banner_art[:-1]: print(line_art.center(term_w_banner_main)); time.sleep(0.03) 
        print(AnsiColors.ENDC); animated_text_display(banner_art[-1], color=AnsiColors.MAGENTA, bold=True, delay=0.005, center=True, width=term_w_banner_main)
        print("\n" * 1); show_spinner(0.8, "Memuat Pengaturan...", color=AnsiColors.GREEN)
        main_menu()
    except KeyboardInterrupt: clear_screen_animated(); animated_text_display(f"{AnsiColors.ORANGE}{AnsiColors.BOLD}🚨 Aplikasi dihentikan paksa. Bye! 🚨{AnsiColors.ENDC}", color=AnsiColors.ORANGE, bold=True, delay=0.01, center=True)
    except Exception as e_fatal:
        clear_screen_animated(); print(f"{AnsiColors.RED}{AnsiColors.BOLD}🔥🔥 Terjadi error fatal: {e_fatal} 🔥🔥{AnsiColors.ENDC}"); import traceback; traceback.print_exc()
        try:
            if logger and logger.handlers: logger.critical("MAIN LEVEL EXCEPTION (FATAL):", exc_info=True)
        except NameError: pass 
        animated_text_display("Tekan Enter untuk keluar...", color=AnsiColors.RED, bold=True, delay=0.01, center=True); input()
