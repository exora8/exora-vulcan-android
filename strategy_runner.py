...
import threading
import requests
import subprocess # BARU: Untuk menjalankan perintah termux
...
termux_api_warning_shown = False # BARU: Flag untuk menampilkan peringatan sekali saja
...
# BARU: Fungsi untuk mengirim notifikasi ke sistem Android via Termux API
def send_termux_notification(title, content):
    global termux_api_warning_shown
    try:
        # Menjalankan perintah termux-notification
        subprocess.run([
            'termux-notification',
            '--title', title,
            '--content', content,
            '--group', 'StrategicAI' # Mengelompokkan notifikasi
        ], check=True, timeout=5)
    except FileNotFoundError:
        # Tampilkan peringatan hanya sekali jika termux-api tidak ditemukan
        if not termux_api_warning_shown:
            print_colored("\n[PERINGATAN] 'termux-notification' tidak ditemukan.", Fore.YELLOW)
            print_colored("Install 'termux-api' via pkg dan aplikasi Termux:API dari F-Droid/Play Store untuk mengaktifkan notifikasi.", Fore.YELLOW)
            termux_api_warning_shown = True
    except subprocess.TimeoutExpired:
        print_colored("\n[ERROR] Perintah notifikasi timeout.", Fore.RED)
...
def display_welcome_message():
    ...
    print_colored("==================================================", Fore.CYAN, Style.BRIGHT)
    print_colored("AI akan mengirim notifikasi saat Open/Close posisi.", Fore.YELLOW)
    print_colored("Pastikan 'termux-api' sudah terpasang.", Fore.YELLOW)
    print()

# ... (Fungsi-fungsi lain yang tidak berubah seperti display_help, manajemen data, dll, tetap sama)
def display_help(): print_colored("\n--- Perintah ---\n!pair <PAIR> [TF]\n!status\n!history\n!exit\n<teks> (untuk chat)", Fore.CYAN)
def load_settings():
    global current_settings, current_instrument_id, DEFAULT_TIMEFRAME
    default_settings = {"groq_api_key": "", "take_profit_pct": 1.0, "stop_loss_pct": 0.5,"min_confidence": 7, "last_pair": None, "last_timeframe": "1H"}
    if os.path.exists(SETTINGS_FILE):
        ...
        key = input("Masukkan Groq API Key Anda: "); current_settings["groq_api_key"] = key; save_settings()
    if not current_settings.get("groq_api_key"): print_colored("ERROR: Groq API Key kosong.", Fore.RED); exit()
    current_instrument_id = current_settings.get("last_pair"); DEFAULT_TIMEFRAME = current_settings.get("last_timeframe")
def save_settings():
    current_settings["last_pair"] = current_instrument_id; current_settings["last_timeframe"] = DEFAULT_TIMEFRAME
...
def display_history():
    ...
    for trade in reversed(autopilot_trades):
        ...
        else: print_colored(f"  Status: {trade['status']}", Fore.YELLOW)
        print()
...
def get_groq_completion(system_prompt, user_content, model='llama3-70b-8192', is_json=False):
    ...
    except requests.exceptions.RequestException as e: print_colored(f"Groq Network Error: {e}", Fore.RED); return None
    except Exception as e: print_colored(f"Groq Response Error: {e}. Resp: {r.text if 'r' in locals() else ''}", Fore.RED); return None

# --- LOGIKA INTI AI (DENGAN NOTIFIKASI) ---

# DIUBAH: Menambahkan panggilan notifikasi saat trade ditutup
async def analyze_and_close_trade(trade, exit_price, close_trigger_reason):
    ...
    system_prompt = "You are a concise trading analyst... (etc.)" # Diringkas
    ...
    is_profit = pnl > MIN_PROFIT_PERCENTAGE
    notif_title = f"✅ TRADE PROFIT: {trade['instrumentId']}" if is_profit else f"❌ TRADE LOSS: {trade['instrumentId']}"
    notif_content = (
        f"P/L: {pnl:.2f}%\n"
        f"Entry: {trade['entryPrice']:.4f} -> Exit: {exit_price:.4f}\n"
        f"Pelajaran: {trade['exitReason']}"
    )
    send_termux_notification(notif_title, notif_content)


# DIUBAH: Menambahkan panggilan notifikasi saat posisi dibuka
async def run_autopilot_analysis():
    ...
    try:
        ...
        if open_position:
            ...
            if (current_settings.get('take_profit_pct') and pnl >= current_settings['take_profit_pct']) or \
               (current_settings.get('stop_loss_pct') and pnl <= -current_settings['stop_loss_pct']):
                close_reason = f"TP/SL Hit ({pnl:.2f}%)"
                await analyze_and_close_trade(open_position, current_price, close_reason)
                is_ai_thinking = False; return
        
        # 2. AI Decision Making
        # ... (Logika prompt engineering & pembelajaran tetap sama persis)
        ...
        # ... (sisanya diringkas karena tidak berubah)
        learning_context = "History:\n" + "\n".join([f"- {t.get('exitReason')}" for t in past_trades]) if past_trades else "No history."
        system_prompt = f"You are a self-correcting trading bot... Learn from this: {learning_context} ... (etc.)"
        user_content = f"Analyze for {current_instrument_id} ... (etc.)"
        ...
        if action == "BUY" and not open_position:
            ...
            if confidence >= current_settings.get("min_confidence", 1):
                ...
                notif_content = (
                    f"Harga Entry: {current_price:.4f}\n"
...
def autopilot_worker():
    while not stop_event.is_set():
        asyncio.run(run_autopilot_analysis()); stop_event.wait(ANALYSIS_INTERVAL_SECONDS)
...
def main():
    ...
    while True:
        try:
            ...
            # ... (penanganan perintah lainnya tetap sama)
