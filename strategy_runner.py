...
is_autopilot_running = True # Default AI menjadi ON saat start
...
# --- INISIALISASI FLASK ---
app = Flask(__name__)
app.config['SECRET_KEY'] = 'supersecretkey' # Diperlukan untuk flash messages jika ingin ditambahkan nanti
...
def display_welcome_message():
    ...
    print_colored("           -- ANIMATED DARK UI EDITION --         ", Fore.YELLOW, Style.BRIGHT)
    print_colored("==================================================", Fore.CYAN, Style.BRIGHT)
    print_colored("Bot trading sedang berjalan di latar belakang.", Fore.GREEN)
    print_colored("Dashboard web UI Anda siap diakses:", Fore.GREEN, Style.BRIGHT)
...
def load_settings():
    ...
    default_settings = {
        ...
        "max_allowed_funding_rate_pct": 0.075, "watched_pairs": {"BTC-USDT": "1H", "ETH-USDT": "1H"} # Default watchlist
    }
...
def save_trades():
    global autopilot_trades
    with state_lock:
        autopilot_trades.sort(key=lambda x: x['entryTimestamp'], reverse=True) # Urutkan dari terbaru
...
# --- API & KALKULASI (Tidak ada perubahan signifikan di sini) ---
...
class LocalAI:
    ...
    def check_for_repeated_mistake(self, current_analysis):
        ...
        for loss in losing_trades:
            ...
            similarity_score = 1; current_pos_vs_ema50 = 'above' if current_analysis['current_candle_close'] > current_analysis['ema50'] else 'below'; past_pos_vs_ema50 = 'above' if past_snapshot['current_candle_close'] > past_snapshot['ema50'] else 'below'
            ...
            avg_solidity_current = sum(current_analysis['pre_entry_candle_solidity']) / 3; past_solidity_list = past_snapshot.get('pre_entry_candle_solidity', [0,0,0]); avg_solidity_past = sum(past_solidity_list) / 3 if past_solidity_list else 0
    ...
    def get_decision(self, candle_data, open_position, funding_rate=0.0):
        ...
        max_funding_rate = self.settings.get("max_allowed_funding_rate_pct", 0.075); potential_trade_type = None
...
async def analyze_and_close_trade(trade, exit_price, reason, is_backtest=False, exit_timestamp_ms=None):
    with state_lock:
        pnl_gross = calculate_pnl(trade['entryPrice'], exit_price, trade.get('type', 'LONG')); exit_dt = datetime.fromtimestamp(exit_timestamp_ms / 1000) if exit_timestamp_ms else datetime.utcnow()
    ...
    if not is_backtest:
        save_trades(); pnl_net = pnl_gross - current_settings.get('fee_pct', 0.1)
        notif_title = f"🔴 Posisi {trade.get('type')} Ditutup: {trade['instrumentId']}"; notif_content = f"PnL (Net): {pnl_net:.2f}% | Exit: {exit_price:.4f} | Trigger: {reason}"
...
async def run_autopilot_analysis(instrument_id):
    ...
    try:
        ...
        relevant_trades = [t for t in autopilot_trades if t['instrumentId'] == instrument_id]; ai = LocalAI(current_settings, relevant_trades); funding_rate = pair_state.get("funding_rate", 0.0)
        ...
        if decision.get('action') in ["BUY", "SELL"] and not open_pos:
            ...
            save_trades(); ai_reason_short = decision.get("reason").split('\n')[0]; notif_title = f"🟢 Posisi {new_trade['type']} Dibuka: {instrument_id}"; notif_content = f"Entry @ {new_trade['entryPrice']:.4f} | {ai_reason_short}"
...
async def check_realtime_position_management(trade_obj, current_candle_data, is_backtest=False):
    ...
    with state_lock:
        if trade_obj['type'] == 'LONG': pnl_at_high = calculate_pnl(trade_obj['entryPrice'], current_candle_data['high'], 'LONG'); pnl_at_low = calculate_pnl(trade_obj['entryPrice'], current_candle_data['low'], 'LONG');
        ...
        elif trade_obj['type'] == 'SHORT': pnl_at_low = calculate_pnl(trade_obj['entryPrice'], current_candle_data['low'], 'SHORT'); pnl_at_high = calculate_pnl(trade_obj['entryPrice'], current_candle_data['high'], 'SHORT');
        ...
        sl_pct = current_settings.get('stop_loss_pct'); sl_price = trade_obj['entryPrice'] * (1 - abs(sl_pct) / 100) if trade_obj['type'] == 'LONG' else trade_obj['entryPrice'] * (1 + abs(sl_pct) / 100)
        if (trade_obj['type'] == 'LONG' and current_candle_data['low'] <= sl_price) or (trade_obj['type'] == 'SHORT' and current_candle_data['high'] >= sl_price): await analyze_and_close_trade(trade_obj, sl_price, f"Stop Loss @ {-abs(sl_pct):.2f}%", is_backtest, current_candle_data['time']); return
        ...
        if trade_obj.get("current_tp_checkpoint_level", 0.0) > 0.0:
            ...
            if ts_price is not None and ((trade_obj['type'] == 'LONG' and current_candle_data['low'] <= ts_price) or (trade_obj['type'] == 'SHORT' and current_candle_data['high'] >= ts_price)): await analyze_and_close_trade(trade_obj, ts_price, f"Trailing TP", is_backtest, current_candle_data['time']); return
        ...
        if pnl_now >= activation_pct:
            ...
            if steps_passed >= 0:
                new_cp = current_cp + (steps_passed * gap_pct); trade_obj['current_tp_checkpoint_level'] = new_cp; new_ts_level = new_cp - gap_pct
...
# --- TEMPLATE HTML BARU DENGAN ANIMASI DAN DESAIN MODERN ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .dashboard-header {
            background: linear-gradient(90deg, #4f00bc, #bf34f5);
            padding: 1.5rem;
            border-radius: 1rem;
            text-align: center;
            color: white;
        ...
        .btn-long { background-image: linear-gradient(to right, #1D976C 0%, #93F9B9  51%, #1D976C  100%);}
        .btn-short { background-image: linear-gradient(to right, #D31027 0%, #EA384D  51%, #D31027  100%);}
        .btn-close-pos { background-image: linear-gradient(to right, #FFB75E 0%, #ED8F03  51%, #FFB75E  100%);}
        .table {
        ...
        .spinner {
            ...
            height: 56px;
            border-radius: 50%;
            background: radial-gradient(farthest-side,#bf34f5 94%,#0000) top/9px 9px no-repeat,
            conic-gradient(#0000 30%,#bf34f5);
...
<body>
    ...
    <div class="container-fluid my-4">
        <div class="dashboard-header">
            <h1 class="display-5"><i class="fas fa-robot"></i> Vulcan Logic AI</h1>
            <p class="lead">Automated Trading & Monitoring Dashboard</p>
        </div>

        <div class="d-flex justify-content-between align-items-center mb-4 px-md-3">
             <h4>AI Autopilot Status: <strong class="{{ 'text-green' if is_ai_running else 'text-red' }}">{{ 'ACTIVE' if is_ai_running else 'INACTIVE' }}</strong></h4>
            <form action="/toggle-ai" method="POST">
                <button type="submit" class="btn-grad">{{ 'DEACTIVATE AI' if is_ai_running else 'ACTIVATE AI' }}</button>
            </form>
        </div>
            ...
            <div class="col-md-4 mb-3">
                <div class="card pnl-card">
                    <div class="card-body">
                        ...
                        <h4 class="card-title {{ 'text-green' if pnl_today > 0 else 'text-red' if pnl_today < 0 else '' }}">{{ "%.2f"|format(pnl_today) }}%</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="card pnl-card">
                    <div class="card-body">
                         <i class="fas fa-calendar-week text-yellow"></i>
                        <h6 class="card-subtitle mb-2 text-muted">This Week's P/L</h6>
                        <h4 class="card-title {{ 'text-green' if pnl_this_week > 0 else 'text-red' if pnl_this_week < 0 else '' }}">{{ "%.2f"|format(pnl_this_week) }}%</h4>
                    </div>
                </div>
            </div>
            <div class="col-md-4 mb-3">
                <div class="card pnl-card">
                     <div class="card-body">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted">Last Week's P/L</h6>
                        <h4 class="card-title {{ 'text-green' if pnl_last_week > 0 else 'text-red' if pnl_last_week < 0 else '' }}">{{ "%.2f"|format(pnl_last_week) }}%</h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Watchlist -->
        <h3 class="mb-3">Market Watchlist</h3>
        <div class="row">
        {% for pair, data in market_data.items() %}
            <div class="col-12 col-md-6 col-xl-4 mb-4">
                <div class="card h-100">
                    <div class="card-body d-flex flex-column">
                        <h4 class="card-title">{{ pair }}</h4>
                        <div class="d-flex justify-content-between mb-2">
                            <span>Price: <strong class="live-data">{{ "%.4f"|format(data.price) }}</strong></span>
                            <span>Funding: <strong class="{{ 'text-red' if data.funding > 0.01 else 'text-green' if data.funding < -0.01 else 'text-muted' }}">{{ "%.4f"|format(data.funding) }}%</strong></span>
                        </div>
                        
                        <div class="mt-auto">
                        {% if data.open_position %}
                            <div class="alert alert-secondary mt-3">
                                <h6 class="alert-heading {{ 'text-green' if data.open_position.type == 'LONG' else 'text-red' }}"><strong><i class="fas fa-folder-open"></i> {{ data.open_position.type }} POSITION</strong></h6>
                                <hr>
                                <div class="d-flex justify-content-between">
                                    <span>Entry Price:</span> <strong>{{ "%.4f"|format(data.open_position.entryPrice) }}</strong>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span>Current P/L:</span> <strong class="live-data {{ 'text-green' if data.pnl > 0 else 'text-red' }}">{{ "%.2f"|format(data.pnl) }}%</strong>
                                </div>
                                 <small class="d-block text-muted text-center mt-1">(Net after 0.1% fee)</small>
                            </div>
                            <form action="/trade/close" method="POST" class="d-grid mt-2">
                                <input type="hidden" name="trade_id" value="{{ data.open_position.id }}">
                                <button type="submit" class="btn btn-grad btn-close-pos">Close Position</button>
                            </form>
                        {% else %}
                            ...
                            <div class="d-flex justify-content-between mt-2">
                                <form action="/trade/manual" method="POST" class="w-50 pe-1">
                                    ...
                                    <button type="submit" class="btn btn-grad btn-long w-100">Long</button>
                                </form>
                                <form action="/trade/manual" method="POST" class="w-50 ps-1">
                                    <input type="hidden" name="pair" value="{{ pair }}">
                                    <input type="hidden" name="type" value="SHORT">
                                    <button type="submit" class="btn btn-grad btn-short w-100">Short</button>
                                </form>
                            </div>
                        {% endif %}
                        </div>
        ...
        <!-- History Table -->
        <h3 class="mt-5 mb-3">Trade History (Last {{ trades|length }})</h3>
        <div class="table-responsive">
            <table class="table table-dark table-hover align-middle">
                <thead>
                    <tr>
                        <th>Pair</th><th>Type</th><th>Status</th><th>Entry Time</th><th>Entry Price</th><th>Exit Price</th><th>P/L (Net)</th><th>Reason</th>
                    </tr>
                </thead>
                <tbody>
                {% for trade in trades %}
                    <tr>
                        ...
                        <td><span class="badge rounded-pill text-bg-{{ 'success' if trade.type == 'LONG' else 'danger' }}">{{ trade.type }}</span></td>
                        <td>{{ trade.status }}</td>
                        <td><small>{{ trade.entryTimestamp.split('T')[0] }}<br>{{ trade.entryTimestamp.split('T')[1].split('.')[0] }}</small></td>
                        ...
                        {% if trade.status == 'CLOSED' %}
                            ...
                            <td class="{{ 'text-green' if pnl_net > 0 else 'text-red' }}"><strong>{{ "%.2f"|format(pnl_net) }}%</strong></td>
                        {% else %}
                            <td>-</td>
                        {% endif %}
    ...
    <script>
        // Script untuk menampilkan loader saat halaman akan di-refresh
...
# --- RUTE FLASK (Tidak ada perubahan logika, hanya memastikan data terkirim) ---
@app.route('/')
def dashboard():
    market_data_view = {}
...
if __name__ == "__main__":
    ...
    # Cleanup saat server dihentikan
